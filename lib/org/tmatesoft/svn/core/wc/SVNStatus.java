 ...
 * method of <b>SVNStatusClient</b>. <b>SVNStatus</b> objects are generated for
 * each 'interesting' local item and depending on the doStatus() method in use
 * either passed for notification to an <b>ISVNStatusHandler</b> implementation
 * or such an object is just returned by the method as a status info for a
 * single item.
 *
 * <p>
 * Within the status handler implementation a developer decides how to interpret
 * status information. For some purposes this way may be more flexible in
 * comparison with calling doStatus() that returns an <b>SVNStatus</b> per one
 * local item. However the latter one may be useful when needing to find out the
 * status of the concrete item.
 *
 * <p>
 *
 ...
 *
 * <br />
 * ...and providing a status handler implementation to an
 * <b>SVNStatusClient</b>'s doStatus() method:
 *
 * <pre class="javacode">
 * ...
 ...
 * </p>
 * <p>
 * <b>SVNStatus</b>'s methods which names start with <code>getRemote</code> are
 * relevant for remote status invocations - that is when a doStatus() method of
 * <b>SVNStatusClient</b> is called with the flag <code>remote</code> set to
 * <span class="javakeyword">true</span>.
 *
 * @version 1.3
...
public class SVNStatus {
     ...
     *
     * @param url
     *            item's repository location
     * @param file
     *            item's path in a File representation
     * @param kind
     *            item's node kind
     * @param revision
     *            item's working revision
     * @param committedRevision
     *            item's last changed revision
     * @param committedDate
     *            item's last changed date
     * @param author
     *            item's last commit author
     * @param contentsStatus
     *            local status of item's contents
     * @param propertiesStatus
     *            local status of item's properties
     * @param remoteContentsStatus
     *            status of item's contents against a repository
     * @param remotePropertiesStatus
     *            status of item's properties against a repository
     * @param isLocked
     *            if the item is locked by the driver (not a user lock)
     * @param isCopied
     *            if the item is added with history
     * @param isSwitched
     *            if the item is switched to a different URL
     * @param isFileExternal
     *            tells if the item is an external file
     ...
     * @param conflictWrkFile
     *            temp file with all user's current local modifications
     * @param projRejectFile
     *            temp file describing properties conflicts
     * @param copyFromURL
     *            url of the item's ancestor from which the item was copied
     * @param copyFromRevision
     *            item's ancestor revision from which the item was copied
     * @param remoteLock
     *            item's lock in the repository
     * @param localLock
     *            item's local lock
     * @param entryProperties
     *            item's SVN specific '&lt;entry' properties
     * @param changelistName
     *            changelist name which the item belongs to
     * @param wcFormatVersion
     *            working copy format number
     * @param treeConflict
     *            tree conflict description
     * @since 1.3
     */
    public SVNStatus(SVNURL url, File file, SVNNodeKind kind, SVNRevision revision, SVNRevision committedRevision, Date committedDate, String author, SVNStatusType contentsStatus,
            SVNStatusType propertiesStatus, SVNStatusType remoteContentsStatus, SVNStatusType remotePropertiesStatus, boolean isLocked, boolean isCopied, boolean isSwitched, boolean isFileExternal,
            File conflictNewFile, File conflictOldFile, File conflictWrkFile, File projRejectFile, String copyFromURL, SVNRevision copyFromRevision, SVNLock remoteLock, SVNLock localLock,
            Map entryProperties, String changelistName, int wcFormatVersion, SVNTreeConflictDescription treeConflict) {
        myURL = url;
        myFile = file;
    ...
    /**
     * Gets the item's latest repository location. For example, the item could
     ...
     * <p>
     * To clean up a Working Copy use {@link SVNWCClient#doCleanup(File)
     * doCleanup()}.
     *
     ...
     *
     * @return <span class="javakeyword">true</span> if the item is added with
     *         history, otherwise <span class="javakeyword">false</span>
     */
    ...
    /**
     * Finds out whether the item is switched to a different repository
     * location.
     *
    ...
    /**
     * Gets the item's SVN specific <i>'&lt;entry'</i> properties. These
     * properties' names start with
     * {@link org.tmatesoft.svn.core.SVNProperty#SVN_ENTRY_PREFIX}.
     *
     * @return a Map which keys are names of SVN entry properties mapped against
     *         their values (both strings)
     */
    ...
    /**
     * Gets the item's last changed date. Relevant for a remote status
     * invocation.
     *
    ...
    /**
     * Gets the item's last changed author. Relevant for a remote status
     * invocation.
     *
    ...
    /**
     * Returns the last modified local time of the file item. Irrelevant for
     * directories (for directories returns <code>Date(0)</code>).
     *
    ...
    /**
     * Sets the item's remote status. Used by SVNKit internals and not intended
     * for users (from an API point of view).
     *
     * @param contents
     *            item's contents status type against the repository
     * @param props
     *            item's properties status type against the repository
     * @param lock
     *            item's lock in the repository
     * @param kind
     *            item's node kind
     ...
     *            item's node kind
     * @param revision
     *            item's latest revision when it was last committed
     * @param date
     *            last item's committed date
     * @param author
    ...
    /**
     * Returns the working copy format number for the admin directory which the
     * statused item is versioned under.
     *
     * <p/>
     * If this status object is a result of a remote status operation, the
