...
class AnytimeSearchTest {
  ...
  @Test
  void matchesBruteForceWithSafeBounds() {
    var choices =
        List.of(
    ...
    var unpruned = AnytimeSearch.maximize(new KnapsackProblem(choices, 3, false), null, () -> true);
    var reduced = AnytimeSearch.maximize(new KnapsackProblem(choices, 3, true), null, () -> true);
  ...
  void retainsTheIncumbentWhenTheBudgetExpires() {
    ...
    var result =
        AnytimeSearch.maximize(new KnapsackProblem(choices, 0, false), incumbent, calls(3));
  ...
  void alwaysUndoesInfeasibleChoices() {
    ...
    var result = AnytimeSearch.maximize(new KnapsackProblem(choices, 0, false), null, () -> true);
  ...
  void restoresStateWhenProblemEvaluationThrows() {
    var problem =
        new KnapsackProblem(List.of(List.of(new Choice("choice", 1, 0))), 0, false) {
          @Override
  ...
  void preservesQualityAcrossEquivalentSearchChanges() {
    ...
    var shortRun = AnytimeSearch.maximize(new KnapsackProblem(choices, 2, false), null, calls(3));
    var longRun = AnytimeSearch.maximize(new KnapsackProblem(choices, 2, false), null, calls(20));
    var expandedRun =
        AnytimeSearch.maximize(new KnapsackProblem(expanded, 2, false), null, () -> true);
    var reorderedRun =
        AnytimeSearch.maximize(
            new KnapsackProblem(choices.stream().map(List::reversed).toList(), 2, false),
            null,
            () -> true);
    var repeatedRun =
        AnytimeSearch.maximize(new KnapsackProblem(choices, 2, false), null, () -> true);
  ...
  private static class KnapsackProblem
    ...
    private KnapsackProblem(List<List<Choice>> choices, int capacity, boolean useBounds) {
