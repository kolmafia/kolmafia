...
public abstract class CombatActionManager {
  ...
  public static final String getCombatAction(
      final String encounter, final int roundIndex, boolean allowMacro) {
        ...
        switch (roundIndex) {
          ...
          case 1:
            return Preferences.getBoolean("autoSteal")
                    && KoLCharacter.hasEquipped(ItemPool.get(ItemPool.NEW_WAVE_BLING, 1))
                ? "try to steal an item"
                : "skip";
          case 2:
            String classStun = CombatActionManager.getStun();
            return Preferences.getBoolean("autoEntangle")
                    && !(KoLCharacter.inClasscore2()
                        && KoLCharacter.getMonsterLevelAdjustment() > 75)
                    && !classStun.equals("none")
                ? classStun
                : "skip";
          case 3:
  ...
  private static String getLongItemAction(final String action) {
    ...
    if (commaIndex != -1) {
      ...
      String second =
          CombatActionManager.getLongItemAction(action.substring(commaIndex + 1).trim());
  ...
  public static final String getShortCombatOptionName(String action) {
    ...
    if (action.equals("stun")) {
      ...
      return name == null || name.equals("none")
          ? "skip"
          : "skill" + SkillDatabase.getSkillId(name);
    }
  ...
  private static String getShortItemAction(final String action) {
    ...
    if (itemId == ItemPool.FACSIMILE_DICTIONARY
        && InventoryManager.getCount(ItemPool.FACSIMILE_DICTIONARY) < 1) {
