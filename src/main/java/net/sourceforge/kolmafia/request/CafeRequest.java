...
public class CafeRequest extends GenericRequest {
  protected static final Pattern CAFE_PATTERN =
      Pattern.compile("cafe.php.*cafeid=(\\d*)", Pattern.DOTALL);
  protected static final Pattern ITEM_PATTERN =
      Pattern.compile("whichitem=(-?\\d*)", Pattern.DOTALL);
  private static final List<Concoction> existing =
      LockableListFactory.getInstance(Concoction.class);
  ...
  public void processResults() {
    ...
    if (this.responseText.contains("You're way too drunk already.")
        || this.responseText.contains("You're too full to eat that.")) {
  ...
  public static final void registerItemUsage(final String itemName, long price) {
    ...
    RequestLogger.updateSessionLog(
        "Buy and " + consume + " 1 " + itemName + " for " + price + " Meat");
