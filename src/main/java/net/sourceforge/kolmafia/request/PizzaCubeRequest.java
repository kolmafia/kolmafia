...
public class PizzaCubeRequest extends GenericRequest {
  private final AdventureResult[] ingredients = new AdventureResult[4];

  /** Constructs a new <code>PizzaCubeRequest</code> */
  public PizzaCubeRequest(
      final AdventureResult item1,
      final AdventureResult item2,
      final AdventureResult item3,
      final AdventureResult item4) {
  ...
  private static AdventureResult[] urlToIngredients(final String urlString) {
    Matcher matcher =
        PizzaCubeRequest.PIZZA_PATTERN.matcher(StringUtilities.getURLDecode(urlString));
    if (!matcher.find()) {
      return null;
    }
