...
public class FantasyRealmRequest extends CreateItemRequest {
  ...
  private static String itemIdToOption(final int itemId) {
    return itemId == ItemPool.FR_WARRIOR_HELM
        ? "1"
        : itemId == ItemPool.FR_MAGE_HAT ? "2" : itemId == ItemPool.FR_ROGUE_MASK ? "3" : "6";
  }
  ...
  public void run() {
    ...
    GenericRequest request =
        new GenericRequest("place.php?whichplace=realm_fantasy&action=fr_initcenter");
    RequestThread.postRequest(request);
    request.constructURLString(
        "choice.php?whichchoice=1280&option="
            + FantasyRealmRequest.itemIdToOption(this.getItemId()));
    RequestThread.postRequest(request);
