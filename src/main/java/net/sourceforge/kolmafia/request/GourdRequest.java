...
public class GourdRequest extends GenericRequest {
  public GourdRequest() {
    this(false);
  }
  ...
  private static final Pattern GOURD1_PATTERN =
      Pattern.compile("Bring back (\\d*)", Pattern.DOTALL);
  private static final Pattern GOURD2_PATTERN =
      Pattern.compile("The (\\d*) <i>urp</i>", Pattern.DOTALL);
  private static final Pattern GOURD3_PATTERN =
      Pattern.compile("value=\"Give him (\\d*)", Pattern.DOTALL);
  ...
  public static final boolean registerRequest(final String urlString) {
    ...
    String message;
    if (urlString.contains("action=gourd")) {
      int count = Preferences.getInteger("gourdItemCount");
      AdventureResult item = GourdRequest.gourdItem(count);
    ...
    } else if (urlString.contains("action=acceptgourdquest")) {
      RequestLogger.printLine("");
      RequestLogger.updateSessionLog();
      message = "Accepting the Gourd Quest";
    } else if (urlString.contains("place=gourd")) {
      RequestLogger.printLine("");
      RequestLogger.updateSessionLog();
