...
public class RecoverCommand extends AbstractCommand {
  ...
  public void run(final String cmd, final String parameters) {
    ...
    try (Checkpoint checkpoint = new Checkpoint()) {
      ...
      if (recoverHP) {
        long target =
            (long) (Preferences.getFloat("hpAutoRecoveryTarget") * KoLCharacter.getMaximumHP());
        RecoveryManager.recoverHP(Math.max(target, KoLCharacter.getCurrentHP() + 1));
      }

      if (recoverMP) {
        long target =
            (long) (Preferences.getFloat("mpAutoRecoveryTarget") * KoLCharacter.getMaximumMP());
