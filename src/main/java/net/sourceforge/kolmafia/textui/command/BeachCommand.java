...
public class BeachCommand extends AbstractCommand {
  public BeachCommand() {
    ...
    if (command.equals("head")) {
      ...
      if (split.length < 2) {
        KoLmafia.updateDisplay(
            MafiaState.ERROR, "Choose which beach head to comb: NUM, EFFECT, or KEYWORD.");
        return;
      }
      ...
      if (split.length == 2) {
        ...
        if (StringUtilities.isNumeric(str)) {
          ...
          if (head == null) {
            KoLmafia.updateDisplay(
                MafiaState.ERROR, "'" + num + "' is not a valid beach head number.");
            return;
          }
        } else {
          ...
          if (head == null) {
            List<String> matchingNames =
                StringUtilities.getMatchingNames(BeachManager.beachHeadDescArray, str);
        ...
        if (head == null) {
          KoLmafia.updateDisplay(
              MafiaState.ERROR, "'" + str + "' doesn't identify a beach head to comb");
          return;
        }
      }
