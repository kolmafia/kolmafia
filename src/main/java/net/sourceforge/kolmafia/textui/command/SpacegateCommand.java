...
public class SpacegateCommand extends AbstractCommand {
  public SpacegateCommand() {
    this.usage =
        " vaccine [#], destination [#######|random] - Perform the specified action at the Spacegate Facility";
  }
  ...
  public void run(final String cmd, final String parameters) {
    if (!Preferences.getBoolean("spacegateAlways") && !Preferences.getBoolean("_spacegateToday")) {
      KoLmafia.updateDisplay(
          MafiaState.ERROR, "You are not cleared to access the Spacegate facility");
      return;
    }
    ...
    if (command.equals("vaccine")) {
      ...
      RequestThread.postRequest(
          new GenericRequest("place.php?whichplace=spacegate&action=sg_vaccinator"));
      ...
      RequestThread.postRequest(
          new GenericRequest("choice.php?whichchoice=1234&option=" + vaccine));
      return;
      ...
      if (destination.equals("random")) {
        RequestThread.postRequest(
            new GenericRequest("place.php?whichplace=spacegate&action=sg_Terminal"));
      ...
      RequestThread.postRequest(
          new GenericRequest("place.php?whichplace=spacegate&action=sg_Terminal"));
      RequestThread.postRequest(
          new GenericRequest("choice.php?whichchoice=1235&option=2&word=" + destination));
      return;
    }
