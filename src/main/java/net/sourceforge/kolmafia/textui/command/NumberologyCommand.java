...
public class NumberologyCommand extends AbstractCommand {
  ...
  public void run(final String cmd, final String parameters) {
    ...
    String prize = NumberologyManager.numberologyPrize(result);
    MafiaState error =
        KoLmafiaCLI.isExecutingCheckOnlyCommand ? MafiaState.CONTINUE : MafiaState.ERROR;
    ...
    if (adventureDelta == 0) {
      if (KoLmafiaCLI.isExecutingCheckOnlyCommand) {
        RequestLogger.printLine(
            "\"numberology " + result + "\" (" + prize + ") is currently available.");
        return;
      }
