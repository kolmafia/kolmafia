...
public class BeachManager {
  ...
  private static final Pattern MINUTES_PATTERN =
      Pattern.compile("You walk for ([\\d,]+) minutes? and find a nice stretch of beach");

  // (You have 11 free walks down the beach left today.)
  private static final Pattern FREE_WALK_PATTERN =
      Pattern.compile("\\(You have ([\\d]+) free walks? down the beach left today.\\)");
  ...
  public static final boolean parseCombUsage(final String urlString, final String text) {
    ...
    if (command == BeachCombCommand.COMB && text.contains("You comb")) {
      ...
      Map<Integer, String> layout =
          BeachManager.stringToLayout(Preferences.getString("_beachLayout"));
  ...
  private static final Pattern MAP_PATTERN =
      Pattern.compile(
          "name=\"coords\" value=\"([\\d]+),([\\d]+)\".*?title=\"([^\"]*).*?otherimages/beachcomb/(.*?).(gif|png)",
          Pattern.DOTALL);
  ...
  public static final void parseBeachMap(final String text) {
    ...
    while (matcher.find()) {
      ...
      char ch =
          type.equals("rough sand with a twinkle")
              ? 't'
              : type.equals("rough sand")
                  ? 'r'
                  : type.equals("combed sand")
                      ? 'c'
                      : type.equals("a beach head")
                          ? 'H'
                          : type.equals("a sand castle") ? 'C' : image.equals("whale") ? 'W' : '?';

      if (ch == '?') {
        logText(
            "Unknown beach square at "
                + minutes
                + ":"
                + row
                + ","
