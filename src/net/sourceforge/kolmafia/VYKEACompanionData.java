...
public class VYKEACompanionData implements Comparable<VYKEACompanionData> {
  ...
  private static final Pattern COMPANION_PATTERN =
      Pattern.compile(
          " *(?:(.*?), the )?level ([12345])(?: (blood|frenzy|lightning))? (bookshelf|ceiling"
              + " fan|couch|dishrack|dresser|lamp) *");
  ...
  private static final Pattern CREATION_PATTERN =
      Pattern.compile(
          "<span class='guts'>.*?It's a (bookshelf|ceiling"
              + " fan|couch|dishrack|dresser|lamp).*?<b>(.*?)</b></span>",
          Pattern.DOTALL);
