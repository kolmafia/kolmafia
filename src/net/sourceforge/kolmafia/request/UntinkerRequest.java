...
public class UntinkerRequest extends GenericRequest {
  ...
  public static final boolean completeQuest() {
    ...
    if (GenericFrame.instanceExists()) {
      if (!InputFieldUtilities.confirm(
          "KoLmafia thinks you haven't completed the screwdriver quest.  Would you like to have"
              + " KoLmafia automatically complete it now?")) {
        return false;
      }
    }
  ...
  public static void decorate(final StringBuffer buffer) {
    ...
    if (KoLCharacter.knollAvailable()) {
      link =
          "<font size=1>[<a href=\"place.php?whichplace=knoll_friendly&action=dk_innabox\">visit"
              + " Innabox</a>]</font>";
    } else {
      link =
