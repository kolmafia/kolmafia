  ...
  public static final boolean allowFoodConsumption(String itemName, int count) {
    ...
    return InputFieldUtilities.confirm(
        "Eating pasta with only "
            + carboLoading
            + " levels of Carboloading will ruin your chance to get the Spaghettihose trophy. Are"
            + " you sure?");
  }
  ...
  public static boolean askAboutMilk(final String name, final int count) {
    ...
    if (!skipLunchNag && canLunch) {
      ...
      if (lunchTurns < consumptionTurns) {
        String message =
            lunchTurns > 0
                ? "Song of the Glorious Lunch will run out before you finish eating that. Are you"
                      + " sure?"
    ...
    if (ConcoctionDatabase.isMayo(id)) {
      if (id == ItemPool.MAYODIOL) {
        if (!InputFieldUtilities.confirm(
            "Putting this in your mouth will cause you to overdrink with the next food, are you"
                + " sure ?")) {
          return false;
        }
      } else {
      ...
      if (Preferences.getString("mayoInMouth").equals("")
        ...
        if (!InputFieldUtilities.confirm(
            "Eating this will cause you to overdrink due to Mayodiol in inventory with Mayo"
                + " Minder&trade; set to use it, are you sure ?")) {
          return false;
        }
      }
      // If we already have Mayodiol in our mouth, warn
        ...
        if (!InputFieldUtilities.confirm(
            "Eating this will cause you to overdrink due to Mayodiol in your mouth, are you"
                + " sure ?")) {
          return false;
        }
      }
