...
public class CampgroundRequest extends GenericRequest {
  ...
  private static final Pattern COLD_MEDICINE_CABINET_PATTERN = Pattern.compile("You can visit the doctors again in (\\d+) turn.<br>You have (\\d) consul");
  ...
  private static void parseWorkshed(final String responseText) {
    ...
    } else if (responseText.contains("You can visit the doctors") || responseText.contains("Looks like the doctors are out for the day")) {
      // Cold Medicine Cabinet usually redirects to choice.php, so this is also handled in ChoiceManager
