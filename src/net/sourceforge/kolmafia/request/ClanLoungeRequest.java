...
public class ClanLoungeRequest extends GenericRequest {
  ...
  private static final Pattern HOTDOG_PATTERN =
      Pattern.compile(
          ".*?<input class=button type=submit value=Eat( disabled.*?)?>.*?<span"
              + " onclick='descitem.\"(.*?)_food\".*?<b>(.*?)</b>(?:.*?<img.*?title=\"(.*?)\"(?:.*?<b>x"
              + " (.*?)</b>.*?([0123456789,]*) in stock)?)?",
          Pattern.DOTALL);
  ...
  private static final Pattern LOUNGE_PATTERN =
      Pattern.compile(
          "<table.*?<b>Clan VIP Lounge \\(Ground"
              + " Floor\\)</b>.*?<center><b>(?:<a.*?>)?(.*?)(?:</a>)?</b>.*?</center>(<table.*?</table>)",
          Pattern.DOTALL);
  private static final Pattern LOUNGE2_PATTERN =
      Pattern.compile(
          "<table.*?<b>Clan VIP Lounge"
              + " \\(Attic\\)</b>.*?<center><b>(?:<a.*?>)?(.*?)(?:</a>)?</b>.*?</center>(<table.*?</table>)",
          Pattern.DOTALL);
