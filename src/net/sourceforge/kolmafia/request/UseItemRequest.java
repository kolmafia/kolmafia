...
public class UseItemRequest extends GenericRequest {
  ...
  private static final Pattern EVILOMETER_PATTERN =
      Pattern.compile(
          "<center>Total evil: <b>(\\d+)</b><p>Alcove: <b>(\\d+)</b><br>Cranny:"
              + " <b>(\\d+)</b><br>Niche: <b>(\\d+)</b><br>Nook: <b>(\\d+)</b></center>");

  private static final Pattern KEYOTRON_PATTERN =
      Pattern.compile(
          "Medbay:</td><td><b>(\\d)/3</b> bio-data segments collected</td></tr><tr><td"
              + " align=right>Waste Processing:</td><td><b>(\\d)/3</b> bio-data segments"
              + " collected</td></tr><tr><td align=right>Sonar:</td><td><b>(\\d)/3</b> bio-data"
              + " segments collected</td></tr><tr><td align=right>Science"
              + " Lab:</td><td><b>(\\d)/6</b> bio-data segments collected</td></tr><tr><td"
              + " align=right>Morgue:</td><td><b>(\\d)/6</b> bio-data segments"
              + " collected</td></tr><tr><td align=right>Special Ops:</td><td><b>(\\d)/6</b>"
              + " bio-data segments collected</td></tr><tr><td"
              + " align=right>Engineering:</td><td><b>(\\d)/9</b> bio-data segments"
              + " collected</td></tr><tr><td align=right>Navigation:</td><td><b>(\\d)/9</b>"
              + " bio-data segments collected</td></tr><tr><td"
              + " align=right>Galley:</td><td><b>(\\d)/9</b> bio-data segments collected");
  ...
  public static final void parseConsumption(final String responseText, final boolean showHTML) {
    ...
    if (responseText.contains("You don't have the item you're trying to use.")) {
      ...
      if (InventoryManager.getCount(itemId) > 0) {
        UseItemRequest.lastUpdate =
            "KoL says don't have that item, but KoLMafia thinks you do, so refreshing KoLMafia"
                + " Inventory";
    ...
    switch (itemId) {
      ...
      case ItemPool.ULTRA_MEGA_SOUR_BALL:
        ...
        if (!responseText.contains(
            "You pop the candy in your mouth, and it immediately absorbs almost all of the moisture"
                + " in your body")) {
          return;
        }
