...
public class UseItemRequest extends GenericRequest {
  ...
  public static final ConsumptionType getConsumptionType(final AdventureResult item) {
    ...
    if (attrs.contains(Attribute.ATTR_USABLE)) {
      return ConsumptionType.USE;
    }
    if (attrs.contains(Attribute.ATTR_MULTIPLE)) {
      return ConsumptionType.USE_MULTIPLE;
    }
    if (attrs.contains(Attribute.ATTR_REUSABLE)) {
  ...
  public void run() {
    ...
    switch (this.consumptionType) {
      ...
      case NONE:
        ...
        if (ItemDatabase.getAttribute(itemId, Attribute.ATTR_CURSE)) {
