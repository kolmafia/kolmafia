...
public class ClanLoungeSwimmingPoolRequest extends GenericRequest {
  // Default to GET_OUT
  ...
  public static void parseResponse(final String urlString, final String responseText) {
    if (!urlString.startsWith("choice.php")
        || !urlString.contains("whichchoice=585")
        || responseText == null) {
      return;
    }
    ...
    if (urlString.indexOf("action=treasure") != -1) {
      ...
      if (swimmingPoolMatcher.find()) {
        RequestLogger.printLine(
            "You found a " + swimmingPoolMatcher.group(1) + " in the VIP pool!");
