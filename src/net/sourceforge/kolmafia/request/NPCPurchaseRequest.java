...
public class NPCPurchaseRequest extends PurchaseRequest {
  ...
  public static final boolean registerShopRequest(final String urlString, boolean meatOnly) {
    ...
    String shopId = ShopRequest.getShopId(urlString);
    if (shopId == null) {
      return false;
    }

    Matcher m = TransferItemRequest.ITEMID_PATTERN.matcher(urlString);
    int itemId = m.find() ? StringUtilities.parseInt(m.group(1)) : parseWhichRow(shopId, urlString);
    ...
    if (priceVal == 0) {
      ...
      // The following are NPCCOIN shops - both NPC shops and Coinmasters
      return switch (shopId) {
        case "armory" -> ArmoryAndLeggeryRequest.registerRequest(urlString, true);
        case "blackmarket" -> BlackMarketRequest.registerRequest(urlString, true);
        case "driparmory" -> DripArmoryRequest.registerRequest(urlString, true);
        case "fdkol" -> FDKOLRequest.registerRequest(urlString, true);
        default -> false;
      };
    }
