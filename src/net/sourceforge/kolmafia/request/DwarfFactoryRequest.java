...
public class DwarfFactoryRequest extends GenericRequest {
  public static final Pattern RUNE_PATTERN =
      Pattern.compile("title=\"Dwarf (Digit|Word) Rune (.)\"");
  ...
  public static final Pattern HOPPER_PATTERN =
      Pattern.compile("<p>It currently contains (\\d+) ([^.]*)\\.</p>");
  ...
  private static final int[] ORES =
      new int[] {
        ItemPool.LINOLEUM_ORE, ItemPool.ASBESTOS_ORE, ItemPool.CHROME_ORE, ItemPool.LUMP_OF_COAL,
      };

  private static final int[] EQUIPMENT =
      new int[] {
        ItemPool.MINERS_HELMET, ItemPool.MINERS_PANTS, ItemPool.MATTOCK,
      };
  ...
  public static final void parseResponse(final String urlString, final String responseText) {
    ...
    if (action.equals("dodice")) {
      ...
      Matcher runeMatcher = DwarfFactoryRequest.getRuneMatcher(responseText);
      String first =
          DwarfFactoryRequest.getRune(runeMatcher) + DwarfFactoryRequest.getRune(runeMatcher);
      String second =
          DwarfFactoryRequest.getRune(runeMatcher) + DwarfFactoryRequest.getRune(runeMatcher);
  ...
  private static void setItemRunes(
      final int itemId, final String rune1, final String rune2, final String rune3) {
  ...
  private static void setEquipmentRune(final String rune) {
    ...
    if (runes.length() == 3) {
      DwarfFactoryRequest.checkForLastRune(
          runes, DwarfFactoryRequest.EQUIPMENT, "pieces of equipment");
    }
  ...
  //
  // Your sporran lights up with a series of four little lights: <color>, <color>, <color>, and
  // <color>.
  //
  // A bunch of little lights on your sporran start flashing random colors like there's a rave on
  // your crotch.

  public static final Pattern DWARF_KILT_PATTERN = Pattern.compile("<p>.*?our sporran.*?</p>");
  ...
  public static void checkOrUse(final int itemId, final boolean use, final StringBuffer output) {
    if (!InventoryManager.hasItem(itemId)) {
      output.append(
          "You do not have the " + ItemDatabase.getItemName(itemId) + KoLConstants.LINE_BREAK);
      return;
    }
  ...
  private static void getItemNumbers(
      final ArrayList<String> list, final int itemId, final int count) {
  ...
  private static void report(final DwarfNumberTranslator digits) {
    ...
    String output =
        "<table border=2 cols=6>"
            ...
            + "<tr>"
            + "<td align=center>"
            + data.getHopperOreCounts(0)
            + "</td>"
            + "<td align=center>"
            + data.getHopperOreCounts(1)
            + "</td>"
            + "<td align=center>"
            + data.getHopperOreCounts(2)
            + "</td>"
            + "<td align=center>"
            + data.getHopperOreCounts(3)
            + "</td>"
            + "</tr>"
            +
            ...
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.HAT, 0)
            + "</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.HAT, 1)
            + "</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.HAT, 2)
            + "</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.HAT, 3)
            + "</td>"
            + "</tr>"
            + "<tr>"
            + "<td align=center>Ores</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.HAT, 0)
            + "</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.HAT, 1)
            + "</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.HAT, 2)
            + "</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.HAT, 3)
            + "</td>"
            + "</tr>"
            +
            ...
            + "<td align=center>Gauges</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.PANTS, 0)
            + "</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.PANTS, 1)
            + "</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.PANTS, 2)
            + "</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.PANTS, 3)
            + "</td>"
            + "</tr>"
            + "<tr>"
            + "<td align=center>Ores</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.PANTS, 0)
            + "</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.PANTS, 1)
            + "</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.PANTS, 2)
            + "</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.PANTS, 3)
            + "</td>"
            + "</tr>"
            +
            ...
            + "<td align=center>Gauges</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.WEAPON, 0)
            + "</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.WEAPON, 1)
            + "</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.WEAPON, 2)
            + "</td>"
            + "<td align=center>"
            + data.getGaugeSetting(FactoryData.WEAPON, 3)
            + "</td>"
            + "</tr>"
            + "<tr>"
            + "<td align=center>Ores</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.WEAPON, 0)
            + "</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.WEAPON, 1)
            + "</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.WEAPON, 2)
            + "</td>"
            + "<td align=center>"
            + data.getOreQuantityString(FactoryData.WEAPON, 3)
            + "</td>"
            + "</tr>"
            + "</table>";
  ...
  public static class DwarfNumberTranslator {
    ...
    private void matchDigitPermutations() {
      ...
      if (this.permutations.size() == 0) {
        ...
        RequestLogger.printLine(
            "Initialized "
                + KoLConstants.COMMA_FORMAT.format(this.permutations.size())
                + " permutations");
      }
      ...
      for (int i = 0; i < this.rolls.size() && this.permutations.size() > 1; ++i) {
        ...
        if (old_perms > new_perms) {
          ...
          RequestLogger.printLine(
              "Eliminated "
                  + KoLConstants.COMMA_FORMAT.format(gone)
                  + " permutations; "
                  + KoLConstants.COMMA_FORMAT.format(new_perms)
                  + " left");
          old_perms = new_perms;
        }
      }
