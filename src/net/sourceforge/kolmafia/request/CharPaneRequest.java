...
public class CharPaneRequest extends GenericRequest {
  ...
  private static Pattern makeConsumptionPattern(final String consumptionString) {
    return Pattern.compile(
        consumptionString
            + ":</span></td><td(?: align=left)?><b><span class=\"(?:blur.)?\">(\\d+) /"
            + " (\\d+)</span>");
  }
  ...
  private static final Pattern PokeFamPattern =
      Pattern.compile(
          "img align=\"absmiddle\""
              + " src=http://images.kingdomofloathing.com/itemimages/(.*?)>&nbsp;(.*?) \\(Lvl"
              + " (\\d+)\\)",
          Pattern.DOTALL);
  ...
  private static final Pattern radSicknessPattern =
      Pattern.compile(
          "Rad(?:iation| Sickness):</td><td align=left><b><font color=black><span alt=\"-(\\d+) to"
              + " All Stats");
