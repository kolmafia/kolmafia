...
public class TelescopeRequest extends GenericRequest {
  private static final Pattern WHERE_PATTERN =
      Pattern.compile("campground.php.*action=telescope([^&]*)");
  ...
  public void run() {
    ...
    if (KoLCharacter.inBadMoon()
        ...
        && KoLCharacter.getTelescopeUpgrades() > 0) {
  ...
  private static final Pattern BEECORE_PATTERN = Pattern.compile("see (.*?) surrounding it.");
  ...
  private static final Pattern[] PATTERNS = {
    ...
    // "You focus the telescope on the back side of the keep, somehow, and see a pipe
    // <description>."
