...
public class VYKEARequest extends CreateItemRequest {
  ...
  public void run() {
    ...
    if (VYKEACompanionData.currentCompanion() != VYKEACompanionData.NO_COMPANION) {
      KoLmafia.updateDisplay(
          MafiaState.ERROR,
          "You already have a VYKEA companion. It would get jealous and turn on you if you build"
              + " another one today.");
      return;
    }
    ...
    if (!InventoryManager.retrieveItem(ItemPool.VYKEA_HEX_KEY, 1)) {
      KoLmafia.updateDisplay(
          MafiaState.ERROR, "You need a VYKEA hex key in order to build a VYKEA companion.");
      return;
    }
    ...
    if (ingredients.length < 3 || ingredients[0].getItemId() != ItemPool.VYKEA_INSTRUCTIONS) {
      KoLmafia.updateDisplay(
          MafiaState.ERROR,
          "VYKEA companion recipe for '" + concoction.getName() + "' is invalid.");
      return;
    }
