...
public class ChateauRequest extends PlaceRequest {
  private static final Pattern PAINTING_PATTERN =
      Pattern.compile("Painting of a[n]? (.*?) \\(1\\)\" title");
  ...
  private static final AdventureResult CHATEAU_SKYLIGHT =
      ItemPool.get(ItemPool.CHATEAU_SKYLIGHT, 1);
  private static final AdventureResult CHATEAU_BANK = ItemPool.get(ItemPool.CHATEAU_BANK, 1);
  private static final AdventureResult CHATEAU_JUICE_BAR =
      ItemPool.get(ItemPool.CHATEAU_JUICE_BAR, 1);
  ...
  public static boolean chateauRestUsable() {
    return Preferences.getBoolean("restUsingChateau") && ChateauRequest.chateauAvailable();
  }
}
