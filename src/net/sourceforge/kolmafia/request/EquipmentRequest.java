...
public class EquipmentRequest extends PasswordHashRequest {
  ...
  private static String chooseEquipmentLocation(final Slot slot) {
    return slot == Slot.HOLSTER
        ? "inventory.php"
        : EquipmentManager.SLOTS.contains(slot)
            ? "inv_equip.php"
            : slot == Slot.CROWNOFTHRONES || slot == Slot.BUDDYBJORN
                ? "bogus.php"
                : EquipmentManager.STICKER_SLOTS.contains(slot)
                    ? "bedazzle.php"
                    : slot == Slot.CARDSLEEVE
                        ? "inv_use.php"
                        : slot == Slot.FAKEHAND
                            ? "inv_equip.php"
                            : EquipmentManager.FOLDER_SLOTS.contains(slot)
                                ? "choice.php"
                                : (slot == Slot.BOOTSKIN
                                        || slot == Slot.BOOTSPUR)
                                    ? "inv_use.php"
  ...
  private void initializeFolderData(final AdventureResult folder, final Slot slot) {
    ...
    if (folder.equals(EquipmentRequest.UNEQUIP)) {
      this.requestType = EquipmentRequestType.REMOVE_ITEM;
      var slotOpt = switch (equipmentSlot) {
        case FOLDER1 -> "0";
        case FOLDER2 -> "1";
        case FOLDER3 -> "2";
        case FOLDER4 -> "3";
  ...
  private void initializeBootData(final AdventureResult decoration, final Slot slot) {
    ...
    if (this.equipmentSlot == Slot.BOOTSKIN
        && this.equipmentType == ConsumptionType.BOOTSPUR) {
      this.error = ItemDatabase.getItemName(this.itemId) + " is a spur, not a skin.";
      return;
    }

    if (this.equipmentSlot == Slot.BOOTSPUR
  ...
  public static final Slot chooseEquipmentSlot(final int itemId) {
    ...
    return switch (equipmentType) {
      ...
      case OFFHAND -> itemId == ItemPool.FAKE_HAND
          ? Slot.FAKEHAND
          : Slot.OFFHAND;
  ...
  private static Slot availableSticker() {
    return EquipmentRequest.availableSlot(EquipmentManager.STICKER_SLOTS);
  }

  public static Slot availableFolder() {
    return EquipmentRequest.availableSlot(KoLCharacter.inHighschool() ? EquipmentManager.FOLDER_SLOTS : EquipmentManager.FOLDER_SLOTS_AFTERCORE);
  }
  ...
  public void run() {
    ...
    if (this.requestType == EquipmentRequestType.CHANGE_ITEM) {
      ...
      if (this.equipmentSlot == Slot.WEAPON
      ...
      if ((EquipmentManager.STICKER_SLOTS.contains(this.equipmentSlot)
              || EquipmentManager.FOLDER_SLOTS.contains(this.equipmentSlot))
    ...
    if (EquipmentManager.FOLDER_SLOTS.contains(this.equipmentSlot)) {
      ...
      case CHANGE_ITEM -> KoLmafia.updateDisplay(
          (this.equipmentSlot == Slot.WEAPON
                  ...
                  : this.equipmentSlot == Slot.OFFHAND
                      ...
                      : this.equipmentSlot == Slot.CARDSLEEVE
                          ? "Sliding in "
                          : this.equipmentSlot == Slot.HOLSTER
                              ? "Holstering "
                              : "Putting on ")
              + ItemDatabase.getItemName(this.itemId)
              + "...");
    ...
    if (changeCardSleeve) {
      RequestThread.postRequest(
          new EquipmentRequest(EquipmentManager.CARD_SLEEVE, Slot.OFFHAND));
    }
  ...
  public void processResults() {
    ...
    if (this.equipmentSlot == Slot.BOOTSKIN
        || this.equipmentSlot == Slot.BOOTSPUR) {
  ...
  private static boolean switchItem(final AdventureResult oldItem, final AdventureResult newItem) {
    ...
    if (oldItemId == ItemPool.SPECIAL_SAUCE_GLOVE
      ...
      EquipmentManager.removeEquipment(
          EquipmentManager.getEquipment(Slot.WEAPON), Slot.WEAPON);
    }
  ...
  public static boolean switchEquipment(
      final Map<Slot, AdventureResult> oldEquipment, final Map<Slot, AdventureResult> newEquipment) {
    ...
    switch (type) {
      ...
      case WEAPON:
        ...
        if (EquipmentDatabase.getHands(newItem.getItemId()) > 1
          ...
          refresh |=
              EquipmentRequest.switchItem(Slot.OFFHAND, EquipmentRequest.UNEQUIP);
        }
        // fall through
      default:
