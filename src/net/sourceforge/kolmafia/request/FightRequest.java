...
public class FightRequest extends GenericRequest {
  ...
  */
  ...
  private static final String[] LEPRECONDO_FURNITURE =
      new String[] {
        "buckets of concrete",
  ...
  private static final Map<String, String> LEPRECONDO_FULFILMENT_TO_NEED =
      Map.ofEntries(
  ...
  static final Pattern LEPRECONDO_NEED = Pattern.compile("is upset that his (.*?) need wasn't met");
  ...
  private static boolean handleLeprecondo(final String text, final String image) {
    ...
    var unmetNeed = LEPRECONDO_NEED.matcher(text);
    if (unmetNeed.find()) {
      processLeprecondoNeed(unmetNeed.group(1));
      return true;
    }

    // Check for met need
    var metNeed =
        LEPRECONDO_FULFILMENT_TO_NEED.entrySet().stream()
            .filter(e -> text.contains(e.getKey()))
            .map(Map.Entry::getValue)
            .findAny()
            .orElse(null);
    if (metNeed != null) {
      processLeprecondoNeed(metNeed);
      return true;
    }
