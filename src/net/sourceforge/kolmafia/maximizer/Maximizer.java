...
public class Maximizer {
  ...
  private static EquipScope emitSlot(
    ...
    if (!Maximizer.eval.codpiece().isSearchable() && SlotSet.CODPIECE_SLOTS.contains(slot)) {
      return equipScope;
    }
    ...
    if (item.equals(EquipmentRequest.UNEQUIP)) {
      ...
      boolean codpieceSlot =
          Maximizer.eval.codpiece().isSearchable() && SlotSet.CODPIECE_SLOTS.contains(slot);
      CheckedItem checkedItem =
          codpieceSlot
              ? new CheckedItem(
                  ...
                  maxPrice,
                  priceLevel,
                  CodpieceMaximizer.maxUsefulGemCopies(itemId),
                  true)
              : new CheckedItem(itemId, equipScope, maxPrice, priceLevel);
      boolean preserveCodpieceGem =
          EquipmentRequest.isCodpieceGem(itemId)
              && Maximizer.eval.codpiece().reservedCopiesInGemSlots(Maximizer.best.equipment, item)
                  > 0;
      ...
      if (equipScope == EquipScope.EQUIP_NOW) {
        ...
        // Track commands which consume or release physical copies.
