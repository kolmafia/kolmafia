...
public class RequestEditorKit extends HTMLEditorKit {
    ...
    if (location.startsWith("account.php")) {
      ...
      if (!InventoryManager.hasItem(ItemPool.TEN_LEAF_CLOVER)) {
        StringUtilities.insertAfter(
            ...
            " onclick=\"return confirm('Are you sure you want to adventure here WITHOUT a ten-leaf"
                + " clover?');\"");
      }
  ...
  protected static final void applyGlobalAdjustments(
    ...
    if (addComplexFeatures) {
      if (RequestEditorKit.maps.contains(location)) {
        buffer.insert(
            buffer.indexOf("</tr>"),
            "<td width=15 valign=bottom align=left bgcolor=blue><a style=\"color: white;"
                + " font-weight: normal; font-size: small; text-decoration: underline\""
                + " href=\"javascript: attachSafetyText(); void(0);\">?</a>");
  ...
  private static void addTransponderLink(final StringBuffer buffer) {
    ...
    String test =
        "You consider beating up Kenneth to see if <i>he</i> remembers it, but you think better of"
            + " it.";
    int index = buffer.indexOf(test);
  ...
  private static final Pattern NAG_PATTERN =
      Pattern.compile(
          "<table.*?Please consider supporting the Kingdom.*?<td height=4>.*?<td"
              + " height=4>.*?</table>",
          Pattern.DOTALL);
  ...
  private static void addOutfitGroup(
    ...
    buffer.append(
        " action=inv_equip.php><input type=hidden name=action value=\"outfit\"><input type=hidden"
            + " name=which value=2><b>");
    buffer.append(label);
    buffer.append(
    ...
    buffer.append(
        "</select></td><td> <input class=button type=submit value=\"Dress"
            + " Up!\"><br></form></td></tr>");
  }
    ...
    if (Preferences.getBoolean("macroLens")) {
      ...
      if (buffer.indexOf(test) == -1) {
        ...
        if (index != -1) {
          buffer.insert(
              index,
              "<form name=macro action=fight.php method=post><input type=hidden name=action"
                  + " value=\"macro\"><input type=\"hidden\" name=\"macrotext\" value=\"\"><tr><td"
                  + " align=center><select name=whichmacro><option value='0'>(select a"
                  + " macro)</option></select> <input class=button type=submit onclick=\"return"
                  + " killforms(this);\" value=\"Execute Macro\"></td></tr></form>");
        }
      }
      StringUtilities.singleStringReplace(
          buffer,
          test,
          "<tr><td><textarea name=\"macrotext\" cols=25 rows=10 placeholder=\"type macro"
              + " here\"></textarea><script language=JavaScript src=\"/"
  ...
  private static final Pattern FOREST_EXPLORATION_PATTERN =
      Pattern.compile(
          "(location on your black map\\.|Halloween falls on a Sunday, maybe\\.|realize why that"
  ...
  private static final Pattern TRASH_PATTERN =
      Pattern.compile(
          "(greasy paper plates|wadded up napkins|discarded bottle caps|empty beer cans|dirty"
              + " plastic cups|empty beer bottles)");
    ...
    switch (choice) {
      ...
      case 1000: // Everything in Moderation
        ...
        StringUtilities.singleStringReplace(
            buffer,
            "rat problem.",
            "rat problem. <font size=1>[<a href=\"tavern.php?place=barkeep\">Visit"
                + " Bart</a>]</font>");
        break;
      case 537:
        // Play Porko!
      ...
      case 579:
        ...
        if (option == 3) {
          ...
          int index =
              buffer.indexOf(
                  "<p><a href=\"adventure.php?snarfblat=280\">Adventure Again (The Hidden"
                      + " Temple)</a>");
          if (index == -1) {
            break;
          }
  ...
  private static void addBugReportWarning(final StringBuffer buffer) {
    ...
    String disclaimer =
        "<p><span style=\"width: 95%; border: 3px solid red; color: red; padding: 5px; text-align:"
            + " left; margin-bottom: 10px; background-color: rgb(254, 226, 226);"
            + " display:block;\">You are currently running in the KoLmafia Relay Browser. It is"
            + " possible that the bug you are experiencing is not in KoL itself, but is a result of"
            + " KoLmafia, a Greasemonkey script, or another client-side modification. The KoL team"
            + " requests that you verify that you can reproduce the bug in a vanilla browser with"
            + " all add-ons and extensions disabled before submitting a bug report.</span>";
  ...
  private static void fixGovernmentLab(final StringBuffer buffer) {
    ...
    link =
        " <a href=\"javascript:singleUse('inv_equip.php',which=2&action=equip&slot=1&whichitem=7770&pwd="
            + GenericRequest.passwordHash
            + "');void();\">[acc1]</a><a"
            + " href=\"javascript:singleUse('inv_equip.php',which=2&action=equip&slot=2&whichitem=7770&pwd="
            + GenericRequest.passwordHash
            + "');void();\">[acc2]</a><a"
            + " href=\"javascript:singleUse('inv_equip.php',which=2&action=equip&slot=3&whichitem=7770&pwd="
            + GenericRequest.passwordHash
            + "');void();\">[acc3]</a>";
    UseLink link1 =
        new UseLink(
