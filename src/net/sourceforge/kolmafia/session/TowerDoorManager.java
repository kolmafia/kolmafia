...
public abstract class TowerDoorManager {
  ...
  public static class Lock {
    ...
    public boolean haveKey() {
      return this.key == null
          || this.key.getCount(KoLConstants.inventory) > 0
          || KoLCharacter.hasEquipped(this.key);
    }
    ...
    public String keyEnchantments() {
      Modifiers mods = ModifierDatabase.getItemModifiers(key.getItemId());
  ...
  private static final Lock[] LOW_KEY_LOCK_DATA = {
    ...
    // Low-Key Locks:
    new Lock(
        "Polka Dotted Lock", ItemPool.CLOWN_CAR_KEY, "key1", "The \"Fun\" House", "Carpool Lane"),
    new Lock(
        "Bat-Winged Lock",
        ItemPool.BATTING_CAGE_KEY,
        "key2",
        "Bat Hole Entrance",
        "Batting a Thousand"),
    new Lock("Taco Locko", ItemPool.AQUI, "key3", "South of the Border", "Lost in Translation"),
    new Lock(
        "Lockenmeyer Flask", ItemPool.KNOB_LABINET_KEY, "key4", "Cobb's Knob Laboratory", "F=ma"),
    new Lock(
        "Antlered Lock",
        ItemPool.WEREMOOSE_KEY,
        "key5",
    ...
    new Lock(
        ...
        "Larrrst & Found"),
    new Lock(
        "Trolling Lock", ItemPool.KEKEKEY, "key7", "The Valley of Rof L'm Fao", "Made of Stars"),
    new Lock(
        "Rabbit-Eared Lock", ItemPool.RABBITS_FOOT_KEY, "key8", "The Dire Warren", "Lucky For You"),
    new Lock(
        "Mine Cart Shaped Lock",
        ItemPool.KNOB_SHAFT_SKATE_KEY,
        "key9",
        "The Knob Shaft",
        "Getting the Shaft Key"),
    new Lock("Frigid Lock", ItemPool.ICE_KEY, "key10", "The Icy Peak", "OF Ice & Yetis"),
    new Lock(
        "Anchovy Can", ItemPool.ANCHOVY_CAN_KEY, "key11", "The Haunted Pantry", "Hola, Amigos"),
    new Lock(
  ...
  // This lock is shaped like a treasure chest, and it tastes like goblins. You had to lick it to
  // figure out what kind of treasure chest it was.
  ...
  public static void parseTowerDoorResponse(final String action, final String responseText) {
    ...
    AdventureResult item =
        responseText.contains("universal key") ? TowerDoorManager.UNIVERSAL_KEY : key;
    ...
    if (responseText.contains("the lock vanishes")
      ...
      Preferences.setString(
          "nsTowerDoorKeysUsed", keys + (keys.equals("") ? "" : ",") + key.getDataName());
    }
  }
  ...
  public static boolean retrieveKey(Lock lock) {
    ...
    if (lock.location != null) {
      KoLmafia.updateDisplay(
          MafiaState.ERROR, "Adventure in " + lock.location + " until you find a " + key);
      return false;
    }
  ...
  public static final void towerDoorScript() {
    ...
    if (needed.size() > 0) {
      ...
      for (Lock lock : needed) {
        ...
        if (!keys.contains(lock.key.getName())) {
          KoLmafia.updateDisplay(
              MafiaState.ERROR, "Failed to open " + lock.name + " using " + lock.key);
          return;
        }
      }
    }
