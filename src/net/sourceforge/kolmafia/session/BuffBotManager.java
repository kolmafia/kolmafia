    ...
    if (failureCount == BuffBotManager.REFUND_THRESHOLD + 1) {
      BuffBotManager.sendRefund(
          ...
          "This message is to provide notification that you have already sent "
              ...
              + "all requests for once-per-day buffs and all buffs which which might result in a refund will instead be treated as donations.",
          0);
    } else if (failureCount < BuffBotManager.REFUND_THRESHOLD) {
      if (!BuffBotHome.isPermitted(requestor)) {
        BuffBotManager.sendRefund(
            requestor,
            "It has been determined that at some point during an attempt to buff you in the last 24 hours, you could not receive buffs.  "
                ...
                + "all of your requests are being refunded rather than processed in order to maintain throughput.  Apologies for the inconvenience.",
            meatSent);
      } else {
        BuffBotManager.sendRefund(
            requestor,
