...
public abstract class BuffBotManager {
  ...
  public static final Pattern MEAT_PATTERN =
      Pattern.compile(
          "<img src=[^>]*?(?:images.kingdomofloathing.com|/images)/itemimages/meat.gif\" height=30"
              + " width=30 alt=\"Meat\">You gain ([\\d,]+) Meat");
  public static final Pattern GIFT1_PATTERN =
      Pattern.compile(
          "<a class=nounder style='color: blue' href='showplayer.php\\?who=(\\d+)'"
              + " target=mainpane>");
  ...
  /**
   * An internal method which adds a buff to the list of available buffs. This also registers the
   * buff inside of the list of available buffs.
   */
  ...
  private static void saveBuffs() {
    ...
    FileUtilities.downloadFile(
        "https://raw.githubusercontent.com/kolmafia/kolmafia/main/data/buffbot.xsl",
        new File(KoLConstants.BUFFBOT_LOCATION, "buffbot.xsl"));
  ...
  /**
   * This is the main BuffBot method. It loops until the user cancels, or an exception (such as not
   * enough MP to continue). On each pass, it gets all messages from the mailbox, then iterates on
   * the mailbox.
   */
  public static final void runBuffBot(final int iterations) {
    ...
    for (int i = iterations; BuffBotHome.isBuffBotActive(); --i) {
      ...
      if (RecoveryManager.getRestoreCount() == 0) {
        if (!usingAdventures || KoLCharacter.getAdventuresLeft() == 0) {
          if (NPCStoreDatabase.contains(ItemPool.MMJ)) {
            AdventureResult restores = ItemPool.get(ItemPool.MMJ, BuffBotManager.initialRestores);
            BuffBotHome.setBuffBotActive(InventoryManager.retrieveItem(restores));
          } else {
            AdventureResult restores =
                ItemPool.get(ItemPool.PHONICS_DOWN, BuffBotManager.initialRestores);
            BuffBotHome.setBuffBotActive(InventoryManager.retrieveItem(restores));
          }
        }
      }
      ...
      BuffBotHome.timeStampedLogEntry(
          BuffBotHome.NOCOLOR, "Message processing complete.  Buffbot is sleeping.");
      if (BuffBotManager.initialRestores > 0) {
        BuffBotHome.timeStampedLogEntry(
            BuffBotHome.NOCOLOR,
            "(" + RecoveryManager.getRestoreCount() + " mana restores remaining)");
  ...
  /**
   * Queues the message to be sent later. Note that only one message can ever be queued. This
   * ensures that thank-you messages do not result in refunds.
   */
  private static void queueOutgoingMessage(
      final String recipient, final String message, final AdventureResult result) {
  ...
  /**
   * Queues an incoming message to be processed. This ensures that the message only appears on one
   * list.
   */
  public static final void queueIncomingMessage(
      final KoLMailMessage message, final boolean delete) {
    if (!BuffBotManager.saveList.contains(message)
        && !BuffBotManager.deleteList.contains(message)) {
      if (delete) {
  ...
  /**
   * Overrides/hides the message handling method in <code>MailManager</code>. Because this is a
   * static final entity, this doesn't really matter, but it is convenient to have, from a style
   * perspective.
   */
  public static final KoLMailMessage addMessage(final String boxname, final String message) {
    ...
    if (!BuffBotManager.sendList.isEmpty()) {
      ...
      BuffBotHome.update(
          BuffBotHome.NOCOLOR,
          "Sending queued message to "
              + ContactManager.getPlayerName(sending.getRecipient())
              + "...");
  ...
  /**
   * Returns whether or not the given username exists on the current white list for restricted
   * buffs.
   */
  ...
  private static void sendRefund(final String recipient, final String reason, final int amount) {
    if (BuffBotManager.sendList.isEmpty()) {
      BuffBotManager.queueOutgoingMessage(
          recipient, reason, new AdventureResult(AdventureResult.MEAT, amount));
      BuffBotHome.update(BuffBotHome.NONBUFFCOLOR, "Queued refund message for [" + recipient + "]");
    }
  }

  /**
   * Checks to see if there's an attached donation by seeing if there's an image tag, with width of
   * 30. Valentine images have width of 100 so we don't mark those as a false positive.
   *
  ...
  private static void sendThankYou(final String recipient, final String messageHTML) {
    if (BuffBotManager.sendList.isEmpty() && !BuffBotManager.thanksMessage.equals("")) {
      ...
      BuffBotManager.queueOutgoingMessage(
          recipient, reason, new AdventureResult(AdventureResult.MEAT, 0));
  ...
  private static Offering extractRequest(final KoLMailMessage message, final int meatSent) {
    ...
    if (meatSent != 0) {
      ...
      BuffBotManager.sendRefund(
          ...
          KoLConstants.COMMA_FORMAT.format(meatSent)
              + " meat is not a valid buff price.  "
              + BuffBotManager.refundMessage,
          meatSent);
      return null;
    }
  ...
  /**
   * Utility method which processes the message that was received. This parses out any applicable
   * buffs and sends any applicable thank you messages.
   */
  private static void processMessage(final KoLMailMessage message) throws Exception {
    // Now that you're guaranteed to be above the threshold,
    // go ahead and process the message.

    BuffBotHome.update(
        BuffBotHome.NONBUFFCOLOR, "Received message from [" + message.getSenderName() + "]");
    ...
    if (failureCount == BuffBotManager.REFUND_THRESHOLD + 1) {
      BuffBotManager.sendRefund(
          requestor,
          "This message is to provide notification that you have already sent "
              + BuffBotManager.REFUND_THRESHOLD
              + " buff requests which resulted in a refund to your account.  In order to preserve"
              + " the integrity of this buffbot, from now until the next rollover begins, all"
              + " requests for once-per-day buffs and all buffs which which might result in a"
              + " refund will instead be treated as donations.",
          0);
    } else if (failureCount < BuffBotManager.REFUND_THRESHOLD) {
      if (!BuffBotHome.isPermitted(requestor)) {
        BuffBotManager.sendRefund(
            requestor,
            "It has been determined that at some point during an attempt to buff you in the last 24"
                + " hours, you could not receive buffs.  This could be either due to engaging in"
                + " combat, having too many AT songs in your head, or ascending before receiving"
                + " your buff.  As a result of this failure, all of your requests are being"
                + " refunded rather than processed in order to maintain throughput.  Apologies for"
                + " the inconvenience.",
            meatSent);
      } else {
        BuffBotManager.sendRefund(
            requestor,
            "This buffbot was unable to process your request.  "
  ...
  private static boolean executeBuff(
      final Offering buff, final String recipient, final int meatSent) {
    ...
    switch (Preferences.getInteger("buffBotPhilanthropyType")) {
      ...
      case 1:
        ...
        if (instanceCount == 0
            || buff.casts.length == 1
                && buff.getLowestBuffId() == SkillPool.ODE_TO_BOOZE
                && instanceCount == 1) {
          break;
        }
      ...
      case 2:
        ...
        BuffBotHome.update(
            BuffBotHome.NONBUFFCOLOR, "Received white list request from un-whitelisted player");
        BuffBotHome.update(
            BuffBotHome.ERRORCOLOR, " ---> Could not cast #" + meatSent + " on " + recipient);
        UseSkillRequest.lastUpdate = "Philanthropic buffs temporarily disabled.";
        return false;
  ...
  public static class Offering implements Comparable<Offering> {
    ...
    public void updateFreeState() {
      ...
      for (int i = 0; i < this.casts.length; ++i) {
        totalCost +=
            this.casts[i]
                * SkillDatabase.getMPConsumptionById(SkillDatabase.getSkillId(this.buffs[i]));
      }

      this.free =
          this.price <= totalCost * 95 / MPRestoreItemList.getManaRestored("magical mystery juice");
    }
    ...
    public boolean castOnTarget(final String target) {
      for (int i = 0; i < this.buffs.length; ++i) {
        ...
        RequestThread.postRequest(
            UseSkillRequest.getInstance(this.buffs[i], target, this.casts[i]));
