...
public class ResultProcessor {
  ...
  public static Pattern ITEM_TABLE_PATTERN =
      Pattern.compile(
          "<table"
              + " class=\"item\".*?rel=\"(.*?)\".*?title=\"(.*?)\".*?descitem\\(([\\d]*)\\).*?</table>");
  public static Pattern BOLD_NAME_PATTERN =
      Pattern.compile(
          "<b>([^<]*)</b>(?: \\((stored in Hagnk's Ancestral Mini-Storage|automatically"
              + " equipped)\\))?");
  ...
  public static Pattern EFFECT_TABLE_PATTERN =
      Pattern.compile(
          "<table><tr><td><img[^>]*eff\\(\"(.*?)\"\\)[^>]*>.*?class=effect>(.*?)<b>(.*?)</b>(?:<br>|"
              + " )?(?:\\((?:duration: )?(\\d+) Adventures?\\))?</td></tr></table>");
