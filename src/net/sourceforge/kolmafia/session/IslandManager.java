...
public class IslandManager {
  ...
  private static final Pattern MAP_PATTERN =
      Pattern.compile("bfleft(\\d*).*bfright(\\d*)", Pattern.DOTALL);
  private static final Pattern JUNKYARD_PATTERN =
      Pattern.compile(
          "(?:The last time I saw my|muttering something about a(?: pair of)?) (.*?)(?:, it was|,"
              + " they were| and) (.*?)[.<]",
          Pattern.DOTALL);
  ...
  private static String sideSummary(
      final String side, final int kills, final int image, int min, final int max) {
    if (kills > min) {
      min = kills;
    }
    int minLeft = 1000 - max;
    int maxLeft = 1000 - min;
    String range = minLeft == maxLeft ? String.valueOf(minLeft) : minLeft + "-" + maxLeft;
  ...
  public static String questCompleter(final String preference) {
    ...
    if (quest.equals("hippy")) return "hippies";
    if (quest.equals("fratboy")) return "fratboys";
    return "none";
  }
  ...
  public static final void addNunneryMeat(final AdventureResult result) {
    ...
    IslandManager.currentNunneryMeat =
    ...
    String message =
        "The nuns take "
            + KoLConstants.COMMA_FORMAT.format(delta)
            + " Meat; "
            + KoLConstants.COMMA_FORMAT.format(recovered)
            + " recovered, "
            + KoLConstants.COMMA_FORMAT.format(remaining)
            + " left to recover.";
  ...
  public static final String areaMessage(final int last, final int current) {
    ...
    final String[] areas =
        IslandManager.fratboy ? IslandManager.HIPPY_AREA_UNLOCK : IslandManager.FRATBOY_AREA_UNLOCK;
  ...
  public static final String heroMessage(final int last, final int current) {
    ...
    final String[] heroes =
        IslandManager.fratboy ? IslandManager.FRATBOY_HERO : IslandManager.HIPPY_HERO;
  ...
  private static void handleEndOfWar(final String loser) {
    ...
    QuestDatabase.Quest warQuest =
        KoLCharacter.isKingdomOfExploathing()
            ? QuestDatabase.Quest.HIPPY_FRAT
            : QuestDatabase.Quest.ISLAND_WAR;
  ...
  public static final void handleBattlefieldMonster(
    ...
    String[][] table =
        IslandManager.fratboy ? IslandManager.FRAT_MESSAGES : IslandManager.HIPPY_MESSAGES;
  ...
  private static final String[][] JUNKYARD_AREAS = {
    {
      "a barrel", "next to that barrel with something burning in it",
    },
    {
      "a refrigerator", "near an abandoned refrigerator",
    },
    {
      "some tires", "over where the old tires are",
    },
    {
      "a car", "out by that rusted-out car",
    },
  };
  ...
  private static void parseOrchard(final String responseText) {
    ...
    String side =
        EquipmentManager.isWearingOutfit(OutfitPool.WAR_HIPPY_OUTFIT) ? "hippy" : "fratboy";
  ...
  private static void parseFarm(final String responseText) {
    ...
    if (responseText.contains("growing soybeans") || responseText.contains("blocks of megatofu")) {
  ...
  public static CoinmasterData findCampMaster(final String urlString) {
    ...
    return camp.equals("1")
        ...
        : camp.equals("2") ? QuartersmasterRequest.FRATBOY : null;
  }
