...
public abstract class ChoiceManager {
  ...
  private static final Pattern WLF_PATTERN =
      Pattern.compile(
          "<form action=choice.php>.*?<b>(.*?)</b>.*?descitem\\((.*?)\\).*?>(.*?)<.*?name=option"
              + " value=([\\d]*).*?</form>",
          Pattern.DOTALL);
  ...
  private static final Pattern VOTE_PATTERN =
      Pattern.compile(
          "<label><input .*? value=\\\"(\\d)\\\" class=\\\"locals\\\" /> (.*?)<br /><span .*?"
              + " color: blue\\\">(.*?)</span><br /></label>");
  private static final Pattern VOTE_SPEECH_PATTERN =
      Pattern.compile(
          "<p><input type='radio' name='g' value='(\\d+)' />"
              + " <b>(.*?)</b>(.*?)<br><blockquote>(.*?)</blockquote>");
  ...
  private static final Pattern DAYCARE_PATTERN =
      Pattern.compile(
          "(?:Looks like|Probably around) (.*?) pieces in all. (.*?) toddlers are training with"
              + " (.*?) instructor");
  ...
  private static final Pattern SAUSAGE_PATTERN =
      Pattern.compile(
          "grinder needs (.*?) of the (.*?) required units of filling to make a sausage.  Your"
              + " grinder reads \\\"(\\d+)\\\" units.");
  ...
  private static final Pattern MUSHROOM_COSTUME_PATTERN =
      Pattern.compile(
          "<form.*?name=option value=(\\d).*?type=submit value=\"(.*?) Costume\".*?>(\\d+)"
              + " coins<.*?</form>");
  ...
  public static final Pattern DECISION_BUTTON_PATTERN =
      Pattern.compile(
          "<input type=hidden name=option value=(\\d+)>(?:.*?)<input +class=button type=submit"
              + " value=\"(.*?)\">");
  ...
  private static final Pattern HELLEVATOR_PATTERN =
      Pattern.compile(
          "the (lobby|first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh)"
              + " (button|floor)");
  ...
  static {
    ChoiceManager.conspiracyQuestMessages.put(
        Quest.CLIPPER,
        "&quot;Attention any available operative. Attention any available operative. A reward has"
            + " been posted for DNA evidence gathered from Lt. Weirdeaux's subjects inside Site 15."
            + " The DNA is to be gathered via keratin extraction. Message repeats.&quot;");
    ChoiceManager.conspiracyQuestMessages.put(
        Quest.EVE,
        "&quot;Attention Operative 01-A-A. General Sitterson reports a... situation involving"
            + " experiment E-V-E-6. Military intervention has been requested. Message"
            + " repeats.&quot;");
    ChoiceManager.conspiracyQuestMessages.put(
        Quest.FAKE_MEDIUM,
        "&quot;Attention Operative EC-T-1. An outside client has expressed interest in the"
            + " acquisition of an ESP suppression collar from the laboratory. Operationally"
            + " significant sums of money are involved. Message repeats.&quot;");
    ChoiceManager.conspiracyQuestMessages.put(
        Quest.GORE,
        "&quot;Attention any available operative. Attention any available operative. Laboratory"
            + " overseer General Sitterson reports unacceptable levels of environmental gore."
            + " Several elevator shafts are already fully clogged, limiting staff mobility, and"
            + " several surveillance camera lenses have been rendered opaque, placing the validity"
            + " of experimental data at risk. Immediate janitorial assistance is requested. Message"
            + " repeats.&quot;");
    ChoiceManager.conspiracyQuestMessages.put(
        Quest.JUNGLE_PUN,
        "&quot;Attention any available operative. Attention any available operative. The director"
            + " of Project Buena Vista has posted a significant bounty for the collection of"
            + " jungle-related puns. Repeat: Jungle-related puns. Non-jungle puns or jungle"
            + " non-puns will not be accepted. Non-jungle non-puns, by order of the director, are"
            + " to be rewarded with summary execution. Message repeats.&quot;");
    ChoiceManager.conspiracyQuestMessages.put(
        Quest.OUT_OF_ORDER,
        "&quot;Attention Operative QZ-N-0. Colonel Kurzweil at Jungle Interior Camp 4 reports the"
            + " theft of Project T. L. B. materials. Requests immediate assistance. Is confident"
            + " that it has not yet been removed from the jungle. Message repeats.&quot;");
    ChoiceManager.conspiracyQuestMessages.put(
        Quest.SERUM,
        "&quot;Attention Operative 21-B-M. Emergency deployment orders have been executed due to a"
            + " shortage of experimental serum P-00. Repeat: P Zero Zero. Lt. Weirdeaux is known to"
            + " have P-00 manufacturing facilities inside the Site 15 mansion. Message"
            + " repeats.&quot;");
    ChoiceManager.conspiracyQuestMessages.put(
        Quest.SMOKES,
        "&quot;Attention Operative 00-A-6. Colonel Kurzweil at Jungle Interior Camp 4 reports that"
            + " they have run out of smokes. Repeat: They have run out of smokes. Requests"
            + " immediate assistance. Message repeats.&quot;");
  }
  ...
  private static final Object[] CHOICE_DATA = {
    ...
    new ChoiceAdventure(
        ...
        new Object[] {
          ...
          new Option(
              "crystalline light bulb + insulated wire + heat-resistant sheet metal ->"
    ...
    new ChoiceAdventure(
        ...
        new Object[] {
          ...
          new Option(
              "acquire gingerbread blackmail photos (drop off fruit-leather negatives and pick up"
                  + " next visit)",
              8,
    ...
    new ChoiceAdventure(
        ...
        new Object[] {
          new Option(
              "Full HP/MP heal, +Mys Exp (20adv), clear partiers (quest), DJ meat (quest),"
                  + " megawoots (quest)",
              1),
  ...
  public static final Pattern MAPPED_MONSTER_PATTERN =
      Pattern.compile(
          "(<input type=\"hidden\" name=\"heyscriptswhatsupwinkwink\" value=\"(\\d+)\" />\\s+<input"
              + " type=\"submit\" class=\"button\" value=\").*?(\" />\\s+</form>)");
      ...
      case 772:
        {
          ...
          result[1] =
              Preferences.getBoolean("_kolhsPoeticallyLicenced")
                  ...
                  : "50 turns of Poetically Licenced (+20% Myst, -20% Muscle, +2 Myst stats/fight,"
                        + " +10% Spell damage)";
          result[2] =
          ...
          result[3] =
              Preferences.getBoolean("_kolhsCutButNotDried")
                  ...
                  : "50 turns of Cut But Not Dried (+20% Muscle, -20% Moxie, +2 Muscle stats/fight,"
                        + " +10% Weapon damage)";
          result[4] =
              Preferences.getBoolean("_kolhsIsskayLikeAnAshtray")
                  ? "Already got Isskay Like An Ashtray today"
                  : "50 turns of Isskay Like An Ashtray (+20% Moxie, -20% Myst, +2 Moxie"
                        + " stats/fight, +10% Pickpocket chance)";
          result[5] = "Make items";
          result[6] = "Make items";
          result[7] = "Make items";
          result[9] = "Leave";
  ...
  private static final Pattern FRAT_RATIONING_PATTERN =
      Pattern.compile(
          "You toss the (.*?) off the edge of the floating battlefield and (\\d+) frat boys? jumps?"
              + " off after it.");
  ...
  private static final Pattern HIPPY_RATIONING_PATTERN =
      Pattern.compile(
          "You toss the (.*?) into the crowd.  (\\d+) hippies dive onto it, greedily consume it,"
              + " and pass out.");
      ...
      case 1448:
        {
          ...
          Matcher stalks =
              Pattern.compile(
                      "<button.*?name=\"pp\""
                          + " value=\"(\\d+)\".*?>\\s+<img.*?src=\".*?/otherimages/powerplant/(\\d+)\\.png\"")
                  .matcher(text);
          while (stalks.find()) {
  ...
  private static void handleSourceTerminal(final String input, String text) {
    if (input.startsWith("educate")) {
      ...
      Pattern CHIPS_PATTERN =
          Pattern.compile(
              "PRAM chips installed: (\\d+).*?GRAM chips installed: (\\d+).*?SPAM chips installed:"
                  + " (\\d+)");
