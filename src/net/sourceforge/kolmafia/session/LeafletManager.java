...
public abstract class LeafletManager {
  ...
  private static final Pattern FOREST_PATTERN =
      Pattern.compile("Gaps in the dense, forbidding foliage lead (.*?),");
  ...
  private static void initialize() throws LeafletException {
    ...
    LeafletManager.sword =
        response.indexOf("An ornate sword") != -1
  ...
  private static void solveLeaflet(final boolean invokeMagic) throws LeafletException {
    ...
    String extra =
        LeafletManager.trophy
            ? " (trophy available)"
            : LeafletManager.magic != null ? " (magic invoked)" : "";
  ...
  private static class LeafletException extends RuntimeException {
