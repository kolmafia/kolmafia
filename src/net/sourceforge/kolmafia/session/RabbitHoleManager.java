...
public abstract class RabbitHoleManager {
  ...
  private static final Pattern SQUARE_PATTERN =
      Pattern.compile(
          "<td.*?background-color:"
              + " #(.*?);.*?title=\"(.*?)\".*?otherimages/chess/(blanktrans|(chess_(.)(.)(.)))\\.gif.*?</td>",
          Pattern.DOTALL);
  ...
  private static class Square {
    ...
    private static final String whiteCompact =
        "style=\"width: 30px; height: 30px; text-align: center; background-color: #ffffff;"
            + " font-size: 30px\"";
    private static final String blackCompact =
        "style=\"width: 30px; height: 30px; text-align: center; background-color: #DDDDDD;"
            + " font-size: 30px\"";
  ...
  private static final String ADVENTURE_AGAIN =
      "<b>Adventure Again:</b></td></tr><tr><td style=\"padding: 5px; border: 1px solid"
          + " blue;\"><center><table><tr><td><center>";
