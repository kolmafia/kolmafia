...
public abstract class SorceressLairManager {
  ...
  private static final AdventureResult[] HEALING_ITEMS =
      new AdventureResult[] {
  ...
  private static final AdventureResult CONFIDENCE =
      EffectPool.get(EffectPool.CONFIDENCE, Integer.MAX_VALUE);
  ...
  private static final String[][] CROWD2_DATA = {
    ...
    {
      "all wearing sunglasses and dancing", "Smoothest Adventurer", Stat.MOXIE.toString(),
    },
  };
  ...
  private static final String[][] CROWD3_DATA = {
    ...
    {
      "people, all of whom appear to be on fire", "Hottest Adventurer", Element.HOT.toString(),
    },
    {
      "people, clustered around a group of igloos", "Coldest Adventurer", Element.COLD.toString(),
    },
    {
  ...
  private static final String[][] MAZE_TRAP1_DATA = {
    // The first maze trap
    {
      "smoldering bushes on the outskirts of a hedge maze", "Hot Damage", Element.HOT.toString(),
    },
    {
      "frost-rimed bushes on the outskirts of a hedge maze", "Cold Damage", Element.COLD.toString(),
    },
    {
    ...
    {
      "purplish, greasy-looking hedges", "Sleaze Damage", Element.SLEAZE.toString(),
    },
  };
  ...
  private static final String[][] MAZE_TRAP2_DATA = {
    ...
    {
      "smoke rising from deeper within the maze", "Hot Damage", Element.HOT.toString(),
    },
    {
      "wintry mists rising from deeper within the maze", "Cold Damage", Element.COLD.toString(),
    },
    {
    ...
    {
      "a cloud of green gas hovering over the maze", "Stench Damage", Element.STENCH.toString(),
    },
    {
  ...
  private static final String[][] MAZE_TRAP3_DATA = {
    // The third maze trap
    {
      "with lava slowly oozing out of it", "Hot Damage", Element.HOT.toString(),
    },
    {
      "occasionally disgorging a bunch of ice cubes", "Cold Damage", Element.COLD.toString(),
    },
    {
      "surrounded by creepy black mist", "Spooky Damage", Element.SPOOKY.toString(),
    },
    {
      "disgorging a really surprising amount of sewage", "Stench Damage", Element.STENCH.toString(),
    },
    {
  ...
  public static void parseChallenge(
      final int challenge, final String test, final String setting1, final String setting2) {
  ...
  private static final Pattern OPTIMISM_PATTERN =
      Pattern.compile("You feel (.*?) about your chances in the (.*?) Adventurer contest");
  private static final Pattern ENTERED_PATTERN =
      Pattern.compile(
          "&quot;You already entered the (.*?) Adventurer contest.*?&quot;", Pattern.DOTALL);
  private static final Pattern QUEUED_PATTERN =
      Pattern.compile("there are (\\d+) (Adventurers|other Adventurers|of them)");
  ...
  private static String contestToStat(final String contest) {
    return contest.equals("Strongest")
        ...
        : contest.equals("Smartest")
            ? Stat.MYSTICALITY.toString()
            : contest.equals("Smoothest") ? Stat.MOXIE.toString() : null;
  }
        ...
        : contest.equals("Coldest")
            ...
            : contest.equals("Spookiest")
                ...
                : contest.equals("Stinkiest")
                    ? Element.STENCH.toString()
                    : contest.equals("Sleaziest") ? Element.SLEAZE.toString() : null;
  }
      ...
      int queue =
          queuedMatcher.find()
              ...
              : (text.contains("you and one other")
                  ...
                  ? 1
                  : text.contains("only Adventurer") || text.contains("only entrant") ? 0 : -1;
            ...
            : responseText.contains("cold damage")
                ...
                : responseText.contains("spooky damage")
                    ...
                    : responseText.contains("stench damage")
                        ? Element.STENCH.toString()
                        : responseText.contains("sleaze damage") ? Element.SLEAZE.toString() : null;
  ...
  private static final String[][] TOWER_DATA = {
    ...
    {
      "nstower_regdesk.gif", QuestDatabase.STARTED,
    },
    {
      "nstower_courtyard.gif", "step3",
    },
    {
      "nstower_hedgemaze.gif", "step4",
    },
    {
      "nstower_towerdoor.gif", "step5",
    },
    {
      "nstower_tower1.gif", "step6",
    },
    {
      "nstower_tower2.gif", "step7",
    },
    {
      "nstower_tower3.gif", "step8",
    },
    {
      "nstower_tower4.gif", "step9",
    },
    {
      "nstower_tower5.gif", "step10",
    },
    {
      "chamberlabel.gif", "step11",
    },
    {
      "kingprism", "step12",
    },
    {
      "gash.gif", QuestDatabase.FINISHED,
    },
  };
        ...
        tag.equals("traps")
            ...
            : (tag.equals("gopher") || tag.equals("duck"))
                ...
                : (tag.equals("chihuahua") || tag.equals("kiwi"))
                    ...
                    : tag.equals("nugglets") ? SorceressLairManager.HEDGE_MAZE_NUGGLETS : 0;
  ...
  private static void hedgeMazeScript(final int mode) {
    ...
    if (mode == SorceressLairManager.HEDGE_MAZE_TRAPS) {
      ...
      if (currentRoom <= 1) {
        ...
        hpLost =
            (int)
                Math.ceil(
                    (double) KoLCharacter.getMaximumHP()
                        * 0.9
                        * (1.0 - KoLCharacter.getElementalResistance(trap1) / 100));
      }
      ...
      if (currentRoom <= 4) {
        ...
        if (trap2 == Element.NONE) {
          if (trap1 != Element.COLD
              && (KoLCharacter.getElementalResistanceLevels(Element.COLD)
                      < KoLCharacter.getElementalResistanceLevels(trap2)
                  || trap2 == Element.NONE)) {
            trap2 = Element.COLD;
          }
          if (trap1 != Element.HOT
              && (KoLCharacter.getElementalResistanceLevels(Element.HOT)
                      < KoLCharacter.getElementalResistanceLevels(trap2)
                  || trap2 == Element.NONE)) {
            trap2 = Element.HOT;
          }
          if (trap1 != Element.SLEAZE
              && KoLCharacter.getElementalResistanceLevels(Element.SLEAZE)
                  < KoLCharacter.getElementalResistanceLevels(trap2)) {
            trap2 = Element.SLEAZE;
          }
          if (trap1 != Element.SPOOKY
              && KoLCharacter.getElementalResistanceLevels(Element.SPOOKY)
                  < KoLCharacter.getElementalResistanceLevels(trap2)) {
            trap2 = Element.SPOOKY;
          }
          if (trap1 != Element.STENCH
        ...
        hpLost +=
            (int)
                Math.ceil(
                    (double) KoLCharacter.getMaximumHP()
                        * 0.8
                        * (1.0 - KoLCharacter.getElementalResistance(trap2) / 100));
      }
        ...
        hpLost +=
            (int)
                Math.ceil(
                    (double) KoLCharacter.getMaximumHP()
                        * 0.7
                        * (1.0 - KoLCharacter.getElementalResistance(trap3) / 100));
      }
      ...
      if (((hpLost >= KoLCharacter.getMaximumHP())
          ...
          && !InputFieldUtilities.confirm(
              "You won't survive to the end of the Hedge Maze, are you sure ?")) {
        return;
      }
    }
    ...
    if (mode != SorceressLairManager.HEDGE_MAZE_NUGGLETS && !KoLCharacter.inDarkGyffte()) {
    ...
    while (status.equals("step4")) {
      ...
      if (mode == SorceressLairManager.HEDGE_MAZE_TRAPS) {
        if (Preferences.getInteger("currentHedgeMazeRoom") == 1
            && !request.responseText.contains("lucky you survived that")) {
          return;
        }
        if (Preferences.getInteger("currentHedgeMazeRoom") == 4
            && !request.responseText.contains("drag yourself out of the opposite end")) {
          return;
        }
        if (Preferences.getInteger("currentHedgeMazeRoom") == 7
            && !request.responseText.contains("emerge from the tunnel")) {
          return;
        }
      }
    ...
    if (status.equals("step5")) {
      ...
      KoLmafia.updateDisplay(
          "Hedge Maze not complete. Unexpected quest status: "
              + Quest.FINAL.getPref()
              + " = "
              + status);
    }
