...
public class ValhallaDecorator {
  public static final void decorateGashJump(final String location, final StringBuffer buffer) {
    ...
    predictions.append(
        "<td><div style=\"padding-top: 10px; padding-left: 10px; padding-right: 10px;"
            + " padding-bottom: 10px\"><font size=-1>");
    ...
    String oldButtons =
        "<input type=submit class=button value=\"Ascend\"> <input type=checkbox name=confirm>"
            + " (confirm) <input type=checkbox name=confirm2> (seriously)";
    String newButtons =
        "<input type=submit class=button value=\"Ascend\"><input type=hidden name=confirm"
            + " value=on><input type=hidden name=confirm2 value=on>";
    ...
    reminders.append(
        "<td bgcolor=\"#eeffee\" valign=top><table><tr><th style=\"text-decoration: underline\""
            + " align=center>Skills You Haven't Yet Permed</th></tr><tr><td align=center><font"
            + " size=\"-1\">");
    ...
    buySkills.append(
        "<td bgcolor=\"#eeffee\" valign=top><table><tr><th style=\"text-decoration: underline\""
            + " align=center>Skills You Didn't Buy</th></tr><tr><td align=center><font"
            + " size=\"-1\">");
    int count =
    ...
    reminders.append(
        "<td bgcolor=\"#eeeeff\" valign=top><table><tr><th style=\"text-decoration: underline\""
            + " align=center>Common Stuff You Didn't Do</th></tr><tr><td align=center><font"
            + " size=\"-1\">");
  ...
  private static void listCommonTasks(final StringBuffer buffer) {
    ...
    if (count > 0) {
      ...
      buffer.append(
          "<nobr><a href=\"javascript:if(confirm('Are you sure you want to discard your Instant"
              + " Karma?')) {singleUse('inventory.php?which=1&action=discard&pwd=");
    ...
    if (InventoryManager.hasItem(ItemPool.CITADEL_SATCHEL)) {
      buffer.append(
          "<nobr><a href=\"guild.php?place=paco\">complete white citadel quest by turning in White"
              + " Citadel Satisfaction Satchel</a></nobr><br>");
    }

    if (InventoryManager.hasItem(ItemPool.THICK_PADDED_ENVELOPE)) {
      buffer.append(
          "<nobr><a href=\"guild.php?place=paco\">complete dwarvish delivery quest by turning in"
              + " thick padded envelope</a></nobr><br>");
    }

    if (InventoryManager.hasItem(ItemPool.DWARVISH_PUNCHCARD)) {
      buffer.append(
          "<nobr><a href=\"dwarfcontraption.php\">acquire dwarvish war outfit"
              + " piece</a></nobr><br>");
    }
    ...
    if (InventoryManager.hasItem(ItemPool.EARTHENWARE_MUFFIN_TIN)
      ...
      buffer.append(
          "<nobr><a href=\"place.php?whichplace=monorail&action=monorail_downtown\">order a muffin"
              + " at the Breakfast Counter</a></nobr><br>");
    }
    ...
    if (InventoryManager.hasItem(ItemPool.BLUEBERRY_MUFFIN)
      ...
      buffer.append(
          "<nobr><a href=\"inventory.php?which=1\">eat your muffin and request a new"
              + " one</a></nobr><br>");
    }
    ...
    if (InventoryManager.hasItem(ItemPool.RAT_WHISKER)
      ...
      buffer.append(
          "<nobr><a"
              + " href=\"place.php?whichplace=town_wrong&action=townwrong_artist_quest&subaction=whiskers\">");
  ...
  private static void switchSeeds(final StringBuffer buffer) {
    ...
    buffer.append(
        "<form style=\"margin: 0; padding: 0; display: inline;\"><select id=\"garden\""
  ...
  private static void switchWorkshed(StringBuffer buffer) {
    ...
    if (canChange) {
      workshedBuffer.append(
          "<form style=\"margin: 0; padding: 0; display: inline;\"><select onchange=\"if"
  ...
  private static void checkIceHouse(StringBuffer buffer) {
    ...
    iceHouseBuffer.append(
        "<nobr>Ice House: <a href=\"museum.php?action=icehouse\" title=\"Check ice house"
            + " monster\">");
    if (monster != null) {
  ...
  private static void switchCowboyBoots(StringBuffer buffer) {
    ...
    cowboyBootsBuffer.append(
        "<form style=\"margin: 0; padding: 0; display: inline;\"><select id=\"cowboy_boots_skin\""
            + " onchange=\"if (this.value) window.location.href=this.value\">");
    cowboyBootsBuffer.append(
    ...
    cowboyBootsBuffer.append(
        "<form style=\"margin: 0; padding: 0; display: inline;\"><select id=\"cowboy_boots_spurs\""
            + " onchange=\"if (this.value) window.location.href=this.value\">");
    cowboyBootsBuffer.append(
        "<option value=\"\" style=\"background-color: #eeeeff\">Apply one</option>");
