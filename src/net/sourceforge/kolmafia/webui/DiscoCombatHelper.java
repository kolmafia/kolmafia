...
public class DiscoCombatHelper {
  ...
  public static final int[] SKILL_ID =
      new int[] {
        50, 51, 52,
      };
  ...
  public static final String[][] COMBOS = {
    {
      "Rave Concentration", "Item Drop +30",
    },
    {
      "Rave Nirvana", "Meat Drop +50",
    },
    {
      "Rave Knockout", "Multi-round stun+damage",
    },
    {
      "Rave Bleeding", "Recurring damage",
    },
    {
      "Rave Steal", "Steal item",
    },
    {
      "Rave Substats", "2-4 substats",
    },
    {
      "Random Rave", "Learn a new combo!",
    },
  };
    ...
    if (combo == RANDOM_RAVE) {
      ...
      for (int sel = 0; sel < 27; ++sel) {
        ...
        for (int test = FIRST_RAVE_COMBO; test < RANDOM_RAVE; ++test) {
          ...
          if (s1 == testdata[0][0] && s2 == testdata[1][0] && s3 == testdata[2][0]) {
  ...
  public static final void parseFightRound(final String action, final Matcher macroMatcher) {
    ...
    for (int i = 0; DiscoCombatHelper.counter > 1 && i < NUM_COMBOS; ++i) {
      ...
      if (DiscoCombatHelper.counter == data.length && DiscoCombatHelper.checkSequence(data, 0)) {
        combo = i;
        DiscoCombatHelper.counter = 0;
        break;
      }
    ...
    if (combo >= 0) {
      String message =
          (combo < FIRST_RAVE_COMBO ? "Disco" : "Rave") + " combo: " + COMBOS[combo][0];
  ...
  private static void learnRaveCombo(int combo) {
    ...
    if (skill1 < FIRST_RAVE_SKILL
        || skill1 > LAST_RAVE_SKILL
        || skill2 < FIRST_RAVE_SKILL
        || skill2 > LAST_RAVE_SKILL
        || skill3 < FIRST_RAVE_SKILL
  ...
  private static void addComboButton(final StringBuffer buffer, final String name, int[] combo) {
    ...
    buffer.append(name);
    buffer.append("\"");
    if (DiscoCombatHelper.counter > 0 && DiscoCombatHelper.counter < 3
        || cost > KoLCharacter.getCurrentMP()) {
  ...
  private static void addDiscoButton(
      final StringBuffer buffer, final int skill, boolean isEnabled) {
