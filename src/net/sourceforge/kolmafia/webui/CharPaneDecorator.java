...
public class CharPaneDecorator {
  ...
  private static final Pattern COMPACT_LASTADV_PATTERN =
      Pattern.compile(
          "<td align=right>(<a onclick=[^<]+ title=\"Last Adventure: ([^\"]+)\" target=mainpane"
              + " href=\"([^\"]+)\">.*?</a>:)</td>");
  ...
  private static void decorateLastAdventure(final StringBuffer buffer) {
    ...
    if (CharPaneRequest.compactCharacterPane) {
      ...
      linkBuffer.append(
          "<span onmouseover=\"document.getElementById('lastadvmenu').style.display = 'inline';\""
              + " onmouseout=\"document.getElementById('lastadvmenu').style.display = 'none';\">");
      ...
      linkBuffer.append(
          " style=\"position: absolute; padding: 5px 5px 5px 5px; background: #f5f5f5; display:"
              + " none\">");
