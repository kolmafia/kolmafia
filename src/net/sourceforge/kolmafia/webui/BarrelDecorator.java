...
public abstract class BarrelDecorator {
  ...
  private static final int[] BARRELROLL = {
    B, S, R, B, R, S, R, S, B, S, B, R, B, R, S, R, B, S, S, R, B, B, R, S, R, S, B, B, S, R, B, R,
    S
  };
  ...
  private static final Pattern UNSMASHED =
      Pattern.compile(
          "smash=(\\d+)&pwd=(\\w+)'><img"
              + " src=[^>]*?(?:images.kingdomofloathing.com|/images)/otherimages/mountains/smallbarrel.gif'.*?>");
  ...
  public static final void decorate(final StringBuffer buffer) {
    ...
    while (m.find()) {
      ...
      m.appendReplacement(
          buffer,
          "smash=$1&pwd=$2'>"
              + "<img src='/images/otherimages/barrels/"
              + filename
              + ".gif' "
              + "border=0 alt=\""
              + tooltip
  ...
  public static final void gainItem(AdventureResult item) {
    ...
    if (ConsumablesDatabase.getInebriety(name) > 0) {
      type = 'B';
    } else if (ConsumablesDatabase.getSpleenHit(name) == 1 && !name.endsWith("egg")) {
      type = 'S';
    }
    // Restorative items aren't nearly so easy to recognize!
    else if (name.startsWith("Doc Galaktik")
        || name.endsWith("seltzer")
        || name.endsWith("Cola")
        || name.endsWith("water")) {
      type = 'R';
    } else {
  ...
  public static final int recommendSquare() {
    if (unsmashedSquares == 0L
        || unsmashedUser
            != KoLCharacter
                .getUserId()) { // need to visit the page to determine which barrels are available
