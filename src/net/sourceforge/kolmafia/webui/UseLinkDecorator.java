  ...
  public static final void decorate(final String location, final StringBuffer buffer) {
    ...
    if (deferrable) {
      ...
      if (inCombat) {
        ...
        if (usedNativeMacro) {
          buffer.append(
              "<form method=POST action=\"account_combatmacros.php\"><input type=HIDDEN name=action"
                  + " value=edit><input type=HIDDEN name=macroid value=\"");
          buffer.append(macro);
  ...
  private static String getSpeculation(String label, Modifiers mods) {
    ...
    String table =
        SpeculateCommand.getHTML(
            ...
            "id='"
                + id
                + "' style='background-color: white; visibility: hidden; position: absolute;"
                + " z-index: 1; right: 0px; top: 1.2em;'");
