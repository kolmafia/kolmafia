...
public class LatteCommand extends AbstractCommand {
  public LatteCommand() {
    this.usage =
        " unlocks | unlocked | refill ingredient1 ingredient2 ingredient2 - Shows unlocks, unlocked"
            + " items, or refills latte";
  }
    ...
    if (command.equalsIgnoreCase("unlocks")) {
      ...
      if (params.length < 4) {
        KoLmafia.updateDisplay(
            MafiaState.ERROR,
            "Use command 'latte refill ingredient1 ingredient2 ingredient3'. Use 'latte unlocked'"
                + " to show available ingredients.");
        return;
      }
      LatteRequest.refill(
