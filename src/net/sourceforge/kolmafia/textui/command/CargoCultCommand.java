...
public class CargoCultCommand extends AbstractCommand {
  public CargoCultCommand() {
    this.usage =
        "[?] # | monster MONSTER | effect EFFECT | item ITEM | stat STAT - pick pocket from your"
            + " shorts"
            + KoLConstants.LINE_BREAK
            + "cargo pocket # - describe contents of specified pocket"
            + KoLConstants.LINE_BREAK
            + "cargo pick # - pick the specified pocket"
            + KoLConstants.LINE_BREAK
            + "cargo count ( type TYPE | unpicked TYPE | monster MONSTER | effect EFFECT | item"
            + " ITEM | stat STAT ) - count matching pockets"
            + KoLConstants.LINE_BREAK
            + "cargo list ( type TYPE | unpicked TYPE |monster MONSTER | effect EFFECT | item ITEM"
            + " | stat STAT ) - list matching pockets"
            + KoLConstants.LINE_BREAK
  ...
  public void run(final String cmd, String parameters) {
    ...
    if (command.equals("demon")) {
      ...
      for (Pocket p : PocketDatabase.scrapSyllables) {
        ...
        RequestLogger.printLine(
            "Pocket #" + pocket + ": " + p.toString() + (syllable == null ? "" : " = " + syllable));
      }
      return;
    }
    ...
    if (command.equals("count") || command.equals("list")) {
      String usage =
          "cargo "
              + command
              + " ( type TYPE | unpicked TYPE | monster MONSTER | item ITEM | effect EFFECT | stat"
              + " STAT )";
      if (split.length < 3) {
  ...
  private MonsterPocket getMonsterPocket(String monsterName) {
    ...
    if (pocket == null) {
      KoLmafia.updateDisplay(
          MafiaState.ERROR, "Your shorts do not contain a monster named '" + monsterName + "'.");
    }
    return pocket;
  }
  ...
  private Set<OneResultPocket> getEffectPockets(String effectName) {
    ...
    if (pockets == null) {
      KoLmafia.updateDisplay(
          MafiaState.ERROR, "Your shorts do not contain an effect named '" + effectName + "'.");
    }
    return pockets;
  }
  ...
  private Set<OneResultPocket> getItemPockets(String itemName) {
    ...
    if (pockets == null) {
      KoLmafia.updateDisplay(
          MafiaState.ERROR, "Your shorts do not contain an item named '" + itemName + "'.");
    }
    return pockets;
  }
