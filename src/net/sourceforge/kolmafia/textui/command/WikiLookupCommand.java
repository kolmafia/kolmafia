...
public class WikiLookupCommand extends AbstractCommand {
  public WikiLookupCommand() {
    this.usage =
        " [effect|familiar|item|skill|outfit|monster|location] <target> - go to appropriate KoL Wiki page. If not specified, defaults to effect then item if no matches.";
  }
  ...
  public void run(final String command, final String parameters) {
    ...
    if (index != -1) {
      ...
      if (type.startsWith("effect")
          || type.startsWith("familiar")
          || type.startsWith("item")
          || type.startsWith("skill")
          || type.startsWith("outfit")
