...
public class FortuneCommand extends AbstractCommand {
  public FortuneCommand() {
    this.usage =
        " - buff mus|mys|mox|familiar|meat|item [word1 word2 word3] | <playername> [word1 word2"
            + " word3] - get a buff or an item, using preference-defined words if none are"
            + " specified.\n"
            + "If playername has spaces, cannot specify words, and does not support playernames"
            + " with 3 spaces";
  }
  ...
  public void run(final String cmd, String parameters) {
    ...
    if (parameters.equals("")) {
      KoLmafia.updateDisplay(
          MafiaState.ERROR, "What do you want to request from the clan fortune teller?");
      return;
    }
    ...
    if (params[0].equals("buff") || params[0].equals("effect")) {
      if (Preferences.getBoolean("_clanFortuneBuffUsed")) {
        KoLmafia.updateDisplay(
            MafiaState.ERROR, "You already received a buff from the clan fortune teller.");
        return;
      }
      ...
      if (Preferences.getInteger("_clanFortuneConsultUses") == 3) {
        KoLmafia.updateDisplay(
            MafiaState.ERROR, "You already consulted with a clanmate 3 times today.");
        return;
      }
      if (params.length == 4) {
        RequestThread.postRequest(
            new ClanFortuneRequest(params[0], params[1], params[2], params[3]));
      } else {
