...
public class CrossStreamsCommand extends AbstractCommand {
  private static final AdventureResult PROTON_ACCELERATOR =
      ItemPool.get(ItemPool.PROTON_ACCELERATOR, 1);

  public CrossStreamsCommand() {
    this.usage =
        " [ <target> ] - Cross streams with the target, default target in preference \"streamCrossDefaultTarget\"";
  }
  ...
  public void run(final String cmd, String parameters) {
    Boolean equipped =
        KoLCharacter.hasEquipped(
            CrossStreamsCommand.PROTON_ACCELERATOR, EquipmentManager.CONTAINER);
    ...
    try (Checkpoint checkpoint = new Checkpoint(equipped)) {
      ...
      if (KoLmafia.permitsContinue()) {
        RequestThread.postRequest(
            new GenericRequest("showplayer.php?action=crossthestreams&who=" + targetId));
      }
    }
  }
}
