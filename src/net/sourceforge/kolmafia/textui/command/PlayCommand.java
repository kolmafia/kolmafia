	...
	extends AbstractCommand
{
	...
	static
	{
		...
		PlayCommand.addBuff( DeckOfEveryCardRequest.MAGICIANSHIP.getName(), DeckOfEveryCardRequest.MAGICIANSHIP, buffs );
		PlayCommand.addBuff( "moxie", DeckOfEveryCardRequest.DANCIN_FOOL, buffs );
		PlayCommand.addBuff( DeckOfEveryCardRequest.DANCIN_FOOL.getName(), DeckOfEveryCardRequest.DANCIN_FOOL, buffs );
		...
		PlayCommand.addBuff( DeckOfEveryCardRequest.FORTUNE_OF_THE_WHEEL.getName(), DeckOfEveryCardRequest.FORTUNE_OF_THE_WHEEL, buffs );
	...
	public void run( final String cmd, String parameter )
	{
		...
		else if ( parameter.startsWith( "stat" ) )
		{
			...
			if ( card == null )
			{
				KoLmafia.updateDisplay( MafiaState.ERROR, "Internal error: could not find card for stat " + parameter );
				return;
			}
		}
		else if ( parameter.startsWith( "buff" ) )
		{
			...
			List<String> matchingNames =  StringUtilities.getMatchingNames( PlayCommand.CANONICAL_BUFF_ARRAY, parameter );
			if ( matchingNames.size() == 0 )
			{
			...
			if ( card == null )
			{
				KoLmafia.updateDisplay( MafiaState.ERROR, "Internal error: could not find card for buff " + parameter );
				return;
			}
		}
		else
