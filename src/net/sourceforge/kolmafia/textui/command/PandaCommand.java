...
public class PandaCommand extends AbstractCommand {
  ...
  private static final Pattern COMMAND_PATTERN3 =
      Pattern.compile("^\\s*arena\\s*([^\\s]+)\\s*(.+)\\s*$");

  public PandaCommand() {
    this.usage =
        " moan | temple | comedy <type> | arena <bandmember> <item> - interact with NPCs in Pandamonium";
  }
    ...
    if (request == null && m.find()) {
      ...
      if (member == null) {
        KoLmafia.updateDisplay(
            MafiaState.ERROR, "I don't think \"" + demon + "\" is a member of the band.");
        return;
      }
