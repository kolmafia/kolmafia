...
public abstract class ChatManager {
  ...
  private static final Pattern BUFF_PATTERN =
      Pattern.compile(
          "(?:A layer of |A |An |has played |has fortified you with |has imbued you with |has given"
              + " you the )(.*?) *(?:for you|has been conjured around you|\\.).*?\\(([\\d]*)"
              + " Adventures\\)");
    ...
    if ("".equals(channel) && content.equalsIgnoreCase("update")) {
      if (!Preferences.getBoolean("chatServesUpdates")) {
        KoLmafia.updateDisplay(
            sender
                + " tried to retrieve recent clan chat messages from you, but you have that feature"
                + " disabled.");
        return;
      }
      if (!ClanManager.isCurrentMember(sender)) {
        KoLmafia.updateDisplay(
            sender
                + " tried to retrieve recent clan chat messages from you, but they are not in your"
                + " clan.");
        return;
      }
