	...
	implements ISVNStatusHandler, ISVNEventHandler
{
	...
	public void handleStatus( SVNStatus status )
	{
		...
		{
			/*
			 * The file, directory or symbolic link item is not under version control, and is configured to be Ignored
			 * during 'add', 'import' and 'status' operations.
			 */
			pathChangeType = "I";
		}
		else if ( contentsStatus == SVNStatusType.STATUS_MISSING || contentsStatus == SVNStatusType.STATUS_INCOMPLETE )
		{
			 ...
			 * The item can be missing if it is removed using a command incompatible with the native Subversion command
			 * line client (for example, just removed from the filesystem). In the case the item is a directory, it can
			 * be incomplete if the user happened to interrupt a checkout or update.
			 */
			pathChangeType = "!";
		}
		...
		{
			/*
			 * The file, directory or symbolic link item is in the repository as one kind of object, but what's actually
			 * in the user's working copy is some other kind. For example, Subversion might have a file in the
			 * repository, but the user removed the file and created a directory in its place, without using the 'svn
			 * delete' or 'svn add' command (or SVNKit analogues for them).
			 */
			pathChangeType = "~";
		}
		else if ( contentsStatus == SVNStatusType.STATUS_REPLACED )
		{
			 ...
			 * deleted, and a new item with the same name was added (within a single revision). While they may have the
			 * same name, the repository considers them to be distinct objects with distinct histories.
			 */
			pathChangeType = "R";
		}
		...
		/*
		 * If SVNStatusClient.doStatus(..) was invoked with remote = true the following code finds out whether the
		 * current item had been changed in the repository
		 */
		...
		/*
		 * Now getting the status of properties of an item. SVNStatusType also contains information on the properties
		 * state.
		 */
		...
		/*
		 * Whether the item was locked in the .svn working area (for example, during a commit or maybe the previous
		 * operation was interrupted, in this case the lock needs to be cleaned up).
		 */
		...
		/*
		 * If doStatus() was run with remote=true and the item is a file, checks whether a remote lock presents.
		 */
		...
		if ( localLock != null )
		{
			...
			{
				/*
				 * if the lock-token of the local lock differs from the lock- token of the remote lock - the lock was
				 * sTolen!
				 */
			...
			else
			{
				...
				{
					/*
					 * the local lock presents but there's no lock in the repository - the lock was Broken. This is true
					 * only if doStatus() was invoked with remote=true.
					 */
					lockLabel = "B";
				}
			}
		...
		{
			/*
			 * the file is not locally locked but locked in the repository - the lock token is in some Other working
			 * copy.
			 */
			lockLabel = "O";
		}
		...
		/*
		 * status is shown in the manner of the native Subversion command line client's command "svn status"
		 */
		RequestLogger.printLine( pathChangeType + propertiesChangeType + ( isLocked ? "L" : " " ) +
