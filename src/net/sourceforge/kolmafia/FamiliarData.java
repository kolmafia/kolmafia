...
public class FamiliarData implements Comparable<FamiliarData> {
  ...
  private static final Pattern CURRENT_FAMILIAR_PATTERN =
      Pattern.compile(
          ".*?(?:onClick='fam\\((\\d+)\\)')?"
              + " .*?src=\"[^>]*?(?:images.kingdomofloathing.com|/images)/(?:item|other)images/([^\"]*?)\".*?<b>(.*?)</b>.*?\\d+-pound"
              + " (.*?) \\(([\\d,]+) (?:exp|experience|candy|candies)?, ([\\d,]+)"
              + " kills?\\)(.*?)(?:</form)>");
  ...
  private static final Pattern FAMILIAR_PATTERN =
      Pattern.compile(
          ".*?name=newfam value=(\\d+).*?<img"
              + " .*?src=\"[^>]*?/(?:item|other)images/([^\"]*?)\".*?>.*?<b>(.*?)</b>.*?\\d+-pound"
              + " (.*?) \\(([\\d,]+) (?:exp|experience|candy|candies)?, ([\\d,]+)"
              + " kills?\\)(.*?)(?:</tr>)");
