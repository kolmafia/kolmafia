...
public class ScriptManageFrame extends GenericPanelFrame {
  ...
  private class InstallScriptsPanel extends GenericPanel {
    ...
    private JPanel layoutTopPanel() {
      ...
      String tooltip =
          "<html>On this panel you can install community-created scripts.<br>Scripts so installed"
              + " will automatically receive available updates when you perform the \"svn update\""
              + " command<br>(which can be set to automatically run on login - look in General >"
              + " Preferences > SVN).<br><br>Notable things you can do here:<br><ul><li>Click a"
              + " header column to sort ascending/descending</li><li>Left-click a script to see"
              + " more details on the bottom panel</li><li>Right-click to install, go to the"
              + " author's kolmafia.us forum thread, and/or see additional"
              + " options</li></ul></html>";
