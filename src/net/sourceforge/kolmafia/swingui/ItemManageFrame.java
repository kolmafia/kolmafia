...
public class ItemManageFrame extends GenericFrame {
  ...
  private class JunkItemsPanel extends OverlapPanel {
    ...
    public void actionCancelled() {
      InputFieldUtilities.alert(
          "These items have been flagged as \"junk\" because at some point in the past, you've"
              + " opted to autosell all of the item.  If you use the \"cleanup\" command, KoLmafia"
              + " will dispose of these items either by pulverizing them (equipment) or autoselling"
              + " them (non-equipment).");
    }
  }
  ...
  private class SingletonItemsPanel extends OverlapPanel {
    ...
    public void actionCancelled() {
      InputFieldUtilities.alert(
          "These items are flagged as \"singletons\".  Using the \"closet\" button, KoLmafia will"
              + " try to ensure that at least one of the item exists in your closet.\n\n"
              + "IF THE PLAYER IS STILL IN HARDCORE OR RONIN, these items are treated as a special"
              + " class of junk items where during the \"cleanup\" routine mentioned in the junk"
              + " tab, KoLmafia will attempt to leave one of the item in the players inventory.\n\n"
              + "Please take note that once the player breaks Ronin, KoLmafia will treat these"
              + " items as normal junk and ignore the general preservation rule.");
    }
  }
  ...
  private class MementoItemsPanel extends OverlapPanel {
    ...
    public void actionCancelled() {
      InputFieldUtilities.alert(
          "These items are flagged as \"mementos\".  IF YOU SET A PREFERENCE, KoLmafia will never"
              + " sell or pulverize these items.");
    }
  }
  ...
  private class RestockPanel extends OverlapPanel {
    ...
    public void actionConfirmed() {
      if (!InputFieldUtilities.confirm(
          "ALL OF THE ITEMS IN THIS LIST, not just the ones you've selected, will be placed into"
              + " your store.  Are you sure you wish to continue?")) {
        return;
      }
      ...
      int selected =
          JOptionPane.showConfirmDialog(
              ...
              StringUtilities.basicTextWrap(
                  "KoLmafia will place all tradeable, autosellable items into your store at"
                      + " 999,999,999 meat. "
