...
public class OptionsFrame extends GenericFrame {
  ...
  private class RelayOptionsPanel extends OptionsPanel {
    ...
    public RelayOptionsPanel() {
      ...
      String[][] options = {
        ...
        {
          "relayShowWarnings",
          "Warn if about to adventure somewhere which contains common adventures you are unprepared"
              + " for or can unlock but haven't"
        },
        {},
        {
  ...
  private class ItemOptionsPanel extends OptionsPanel {
    ...
    public ItemOptionsPanel() {
      ...
      String helpText =
          "The following settings control \"automated\" item acquisition: the \"acquire\" command"
              + " and other commands, such as \"eat\", \"equip\", or \"use\", that use \"acquire\""
              + " to get items into inventory for further manipulation.";
  ...
  private class IotMTrackingPanel extends OptionsPanel {
    ...
    public IotMTrackingPanel() {
      ...
      String helpText =
          "Some Items Of The Month have daily passes, and so KoLMafia cannot tell if you have them"
              + " from seeing the zone. You can mark them here instead.";
  ...
  protected class StartupFramesPanel extends GenericPanel implements ListDataListener {
    ...
    public StartupFramesPanel() {
      ...
      JTextArea message =
          new JTextArea(
              "These are the global settings for what shows up when KoLmafia successfully logs into"
                  + " the Kingdom of Loathing.  You can drag and drop options in the lists below to"
                  + " customize what will show up.\n\n"
                  + "When you place the Local Relay Server into the 'startup in tabs' section,"
                  + " KoLmafia will start up the server but not open your browser.  When you place"
                  + " the Contact List into the 'startup in tabs' section, KoLmafia will force a"
                  + " refresh of your contact list on login.\n");
  ...
  private class DeedsButtonPanel extends ScrollablePanel implements ListDataListener {
    ...
    private class HelpRunnable implements Runnable {
      ...
      public HelpRunnable() {
        String message =
            "<html><table width=750><tr><td><b>NOTE:</b> If you want to use the Custom Deed"
                + " Builder, click the \"Add Custom\" button; the following instructions are used"
                + " for manually editing the dailyDeedsOptions user preference.<br>"
                + "------------------------<br>All deeds are specified by one comma-delimited"
                + " preference \"dailyDeedsOptions\".  Order matters.  Built-in deeds are simply"
                + " called by referring to their built-in name; these are viewable by pulling up"
                + " the Daily Deeds tab and looking in the \"Built-in Deeds\" list.<h3><b>Custom"
                + " Deeds</b></h3>Custom deeds provide the user with a way of adding buttons or"
                + " text to their daily deeds panel that is not natively provided for.  All deeds"
                + " start with the keyword <b>$CUSTOM</b> followed by a pipe (|) symbol.  As you"
                + " are constructing a custom deed, you separate the different arguments with"
                + " pipes.<br><br>All deed types except for Text require a preference to track.  If"
                + " you want to add a button that is always enabled, you will have to create a"
                + " dummy preference that is always false.<br><br>There are currently 6 different"
                + " types of custom deeds.  Remember that all of these \"acceptable forms\" are"
                + " prefaced by $CUSTOM|.<br><br><b>Simple</b> - Designed for users who do not want"
                + " to bother with preferences.  Will be disabled after the first"
                + " click.<br>acceptable forms:<br>Simple|displayText<br>"
                + "Simple|displayText|command<br>Simple|displayText|command|maxUses<br>"
                + "<br>displayText - the text that will be displayed on the button<br>command - the"
                + " command to execute. If not specified, will default to displayText.<br>maxUses -"
                + " an arbitrary integer. Specifies a threshold to disable the button at. A counter"
                + " in the form of <clicks>/<maxUses> will be displayed to the right of the button."
                + " <br> After clicking a simple deed, the button will be immediately disabled"
                + " unless maxUses>1 is specified.<br><br><b>Command</b> - execute a command with a"
                + " button press<br>acceptable forms:<br>Command|displayText|preference<br>"
                + "Command|displayText|preference|command<br>"
                + "Command|displayText|preference|command|maxUses<br><br>displayText - the text"
                + " that will be displayed on the button<br>preference - the preference to track. "
                + " The button will be enabled when the preference is less than maxUses (default"
                + " 1).<br>command - the command to execute.  If not specified, will default to"
                + " displayText.<br>maxUses - an arbitrary integer.  Specifies a threshold to"
                + " disable the button at.  A counter in the form of"
                + " &lt;preference&gt;/&lt;maxUses&gt; will be displayed to the right of the"
                + " button.<br><br><b>Item</b> - this button will use fuzzy matching to find the"
                + " name of the item specified.  Will execute \"use &lt;itemName&gt;\" when"
                + " clicked.  Will only be visible when you possess one or more of the"
                + " item.<br>acceptable forms:<br>Item|displayText|preference<br>"
                + "Item|displayText|preference|itemName<br>"
                + "Item|displayText|preference|itemName|maxUses<br><br>itemName - the name of the"
                + " item that will be used.  If not specified, will default to displayText.<br>"
                + "<br><b>Skill</b> - cast a skill that is tracked by a boolean or int preference. "
                + " Will execute \"cast &lt;skillName&gt;\" when clicked.  Will not be visible if"
                + " you don't know the skill.<br>acceptable forms:<br>"
                + "Skill|displayText|preference<br>Skill|displayText|preference|skillName<br>"
                + "Skill|displayText|preference|skillName|maxCasts<br><br>skillName- the name of"
                + " the skill that will be cast.  If not specified, will default to displayText. "
                + " Must be specified if maxCasts are specified.<br>maxCasts - an arbitrary"
                + " integer.  Specifies a threshold to disable the button at.  A counter in the"
                + " form of &lt;preference&gt;/&lt;maxCasts&gt; will be displayed to the right of"
                + " the button.<br><br><b>Text</b><br>acceptable forms:<br>Text|pretty much"
                + " anything.<br><br>You can supply as many arguments as you want to a Text deed. "
                + " Any argument that uniquely matches a preference will be replaced by that"
                + " preference's value.  If you want to use a comma in your text, immediately"
                + " follow the comma with a pipe character so it will not be parsed as the end of"
                + " the Text deed.<br><br><b>Combo</b> - A cleaner way to collapse multiple related"
                + " command deeds into one combobox element. Note that there is no GUI to help you"
                + " construct this, you must manually add it to your dailyDeedsOptions"
                + " preference.<br>acceptable forms:<br>"
                + "$CUSTOM|Combo|displayText|preference1|itemBlock<br>"
                + "$CUSTOM|Combo|displayText|preference1|maxUses|itemBlock<br>where the itemBlock"
                + " consists of an arbitrary number of:<br>"
                + "$ITEM|displayText|preferenceN|command|<br><br>Preference1 - The preference to"
                + " track for enabling/disabling the entire combobox. Default threshold is 1 if"
                + " maxUses is not specified.<br>Preference2 - The individual preference to disable"
                + " each individual item in the combobox. Note that there is no way to supply"
                + " maxUses for each individual element; 1 (or true) is always the max. "
                + "</td></tr></table></html>";
  ...
  private class SVNPanel extends JPanel {
    ...
    public SVNPanel() {
      ...
      JTextArea message =
          new JTextArea(
              "Configure the behavior of Mafia's built-in SVN client here.\n\n"
                  + "With SVN you can seamlessly install community-created scripts and have them"
      ...
      tip =
          "<html>A script may declare dependencies - i.e. other scripts that should be"
              + " installed<br>along with it.  Those dependencies can declare their own"
              + " dependencies, and<br>so forth.<br><br>Users who want complete control over what"
              + " they are installing can turn this off,<br>but should make sure to manually"
              + " install dependencies or scripts may<br>malfunction.";
      this.queue(
          new PreferenceCheckBox(
      ...
      tip =
          "<html>If you manually modify your working copies, syncing will ensure that"
              + " any<br>changes you made in the WC (along with merged updates from the repo)"
              + " are<br>transferred to your local copy whenever you perform an svn update.  This"
              + " is<br>equivalent to executing the CLI command <i>svn sync</i>.<br><br>If you have"
              + " not modified your working copies, you do not need this feature.</html>";
  ...
  protected class BrowserPanel extends OptionsPanel {
    ...
    public BrowserPanel() {
      ...
      if (KoLConstants.USE_OSX_STYLE_DIRECTORIES) {
        ...
        path = "/Applications";
        helpText =
            "If KoLmafia opens a browser other than your default, enter the name of your preferred"
                + " browser here. The browser must be in your Applications directory";
      } else if (KoLConstants.USE_LINUX_STYLE_DIRECTORIES) {
        // button = true;
        path = "/";
        helpText =
            "If KoLmafia opens a browser other than your default, enter the name of your preferred"
                + " browser here. If that doesn't work, click the button and browse to the location"
                + " of your browser.";
      } else
      // Windows
      {
