...
public class StoreManageFrame extends GenericPanelFrame {
  ...
  public static final void updateEarnings(final long potentialEarnings) {
    ...
    StoreManageFrame.INSTANCE.setTitle(
        "Store Manager (potential earnings: "
            + KoLConstants.COMMA_FORMAT.format(potentialEarnings)
            + " meat)");
  }
  ...
  private class StoreManagePanel extends GenericPanel {
    ...
    public void actionConfirmed() {
      ...
      for (int i = 0; i < rowCount; ++i) {
        ...
        if (cheapest >= 1000000 && prices[i] < cheapest * 0.15) {
          String message =
              item + ": the price is less than 15% of the cheapest in the mall, continue?";
  ...
  public static final String UNDERCUT_MESSAGE =
      "KoLmafia will take items priced at 999,999,999,999 meat and undercut the current lowest price in the mall.  "
  ...
  protected class StoreManageTable extends ShowDescriptionTable<SoldItem> {
    ...
    private void setRenderers() {
      ...
      IntegerRenderer rend = new IntegerRenderer(); // TODO: do we need LongRenderer?
    ...
    private void setHighlighters() {
      ...
      HighlightPredicate mouseOver =
          new HighlightPredicate() {
            ...
            public boolean isHighlighted(Component renderer, ComponentAdapter adapter) {
              ...
              Point p =
                  (Point) adapter.getComponent().getClientProperty(RolloverProducer.ROLLOVER_KEY);
              return p != null
                  && p.y == adapter.row
                  && convertColumnIndexToModel(p.x) == 1
                  && convertColumnIndexToModel(adapter.column) == 1;
            }
          };
          ...
          new HighlightPredicate() {
            ...
            public boolean isHighlighted(Component renderer, ComponentAdapter adapter) {
              ...
              if (convertColumnIndexToModel(adapter.column) == 1) {
                ...
                SoldItem it =
                    ((StoreManageTableModel) StoreManageTable.this.getModel())
                        .getSoldItem(convertRowIndexToModel(adapter.row));
          ...
          new HighlightPredicate() {
            ...
            public boolean isHighlighted(Component renderer, ComponentAdapter adapter) {
              ...
              SoldItem it =
                  ((StoreManageTableModel) StoreManageTable.this.getModel())
                      .getSoldItem(convertRowIndexToModel(adapter.row));
              return it != null && (cellValue < it.getLowest() * 0.15) && it.getLowest() > 50000L;
            }
          };
      ...
      AbstractHighlighter bold =
          new AbstractHighlighter(auto) {
      ...
      AbstractHighlighter underline =
          new AbstractHighlighter(mouseOver) {
    ...
    private class JButtonHackEditor implements TableCellEditor {
      /*
      ...
      public Component getTableCellEditorComponent(
        ...
        if (value instanceof Boolean || value instanceof Integer) {
          ...
          StoreManageFrame.showGlassBox(
              convertRowIndexToModel(row), p.x + rect.width / 2 - 15, p.y + 13);
  ...
  private class SearchItemListener extends ThreadedListener {
    ...
    protected void execute() {
      ...
      if (searchName == null) {
        AdventureResult item =
            (AdventureResult) StoreManageFrame.this.sellingList.getSelectedItem();
        if (item == null) {
          return;
        }
  ...
  private static class PriceEditor extends DefaultCellEditor {
    private static final AutoHighlightTextField rightField =
...
class LimitGlassBox extends JXGlassBox {
  private long limit;
