
package com.velocityreviews.forums;

import java.io.IOException;
import java.net.Socket;
import java.net.SocketException;
import java.net.URL;
import java.net.UnknownHostException;

import sun.net.www.http.HttpClient;

/**
 * Need to override any function in HttpURLConnection that create a new HttpClient and create a HttpTimeoutClient
 * instead. This source code was originally found at:
 * http://www.velocityreviews.com/forums/t130657-httpurlconnection-timeout-solution.html It has been modified to have
 * timeout be non-configurable for added encapsulation.
 */

public class HttpTimeoutClient
	extends HttpClient
{
	private static int currentTimeout = 30000;

	public HttpTimeoutClient( final URL location )
		throws IOException
	{
		super( location, (String) null, -1 );
	}

	public HttpTimeoutClient( final URL location, final String proxy, final int proxyPort )
		throws IOException
	{
		super( location, proxy, proxyPort );
	}

	public static HttpTimeoutClient getInstance( final URL location )
		throws IOException
	{
		HttpTimeoutClient client = (HttpTimeoutClient) HttpClient.kac.get( location );

		if ( client == null )
		{
			if ( System.getProperty( "proxySet" ) != null && System.getProperty( "proxySet" ).equals( "true" ) )
			{
				client =
					new HttpTimeoutClient(
						location, System.getProperty( "http.proxyHost" ),
						Integer.parseInt( System.getProperty( "http.proxyPort" ) ) );
			}
			else
			{
				client = new HttpTimeoutClient( location ); // CTOR called openServer()
			}
		}
		else
		{
			client.url = location;
		}

		return client;
	}

	public static void setHttpTimeout( final int newTimeout )
	{
		HttpTimeoutClient.currentTimeout = newTimeout;
	}

	// Override doConnect in NetworkClient

	protected Socket doConnect( final String s, final int i )
		throws IOException, UnknownHostException, SocketException
	{
		Socket socket = super.doConnect( s, i );

		// This is the important bit

		socket.setSoTimeout( HttpTimeoutClient.currentTimeout );
		return socket;
	}
}
